...
document.querySelector('#clearProductGallery').onclick=()=>{clearEditedGallery=true;document.querySelector('#productEditForm').galleryPhotos.value='';renderGalleryPreview([])};
document.querySelector('#productEditForm').galleryPhotos.onchange=async e=>{
  const files=[...(e.target.files||[])].slice(0,6);
  if(!files.length)return;
  clearEditedGallery=false;
  const previews=await Promise.all(files.map(file=>fileData(file)));
  renderGalleryPreview(previews);
};
...
document.querySelector('#productEditForm').onsubmit=async e=>{e.preventDefault();const form=e.currentTarget;if(!form.reportValidity())return;const submit=form.querySelector('[type="submit"]'),f=new FormData(form),id=f.get('productId')||`bread-${Date.now()}`,existing=productRegistry.find(x=>x.id===id),isNew=!existing;submit.disabled=true;submit.textContent='Сохраняю…';try{const uploaded=await fileData(form.photo.files[0]);const galleryFiles=[...(form.galleryPhotos.files||[])].slice(0,6);const uploadedGallery=galleryFiles.length?await Promise.all(galleryFiles.map(file=>fileData(file))):[];const p={...(existing||{}),id,active:f.get('active')==='on',storefrontVisible:f.get('storefrontVisible')==='on',builtIn:existing?.builtIn||false,names:{ru:f.get('nameRu').trim(),en:f.get('nameEn').trim(),es:f.get('nameEs').trim()},descriptions:{ru:f.get('descRu').trim(),en:f.get('descEn').trim(),es:f.get('descEs').trim()},weight:Number(f.get('weight')),basePrice:Number(f.get('basePrice')),wholesaleMinQty:Math.max(1,Number(f.get('wholesaleMinQty')||12)),image:uploaded||(!removeEditedPhoto&&existing?.image)||((id==='plain'?'bread-plain.jpg':id==='pumpkin'?'bread-pumpkin.jpg':'icon.svg')),gallery:uploadedGallery.length?uploadedGallery:(clearEditedGallery?[]:(existing?.gallery||[]))};if(!window.panoraCloud?.ready)throw new Error('Облако ещё загружается. Подождите несколько секунд и повторите.');const confirmed=await window.panoraCloud.saveProductConfirmed(p);if(existing)Object.assign(existing,confirmed);else productRegistry.push(confirmed);syncAdminProductRegistry();localStorage.setItem('panora-products',JSON.stringify(productRegistry));window.dispatchEvent(new CustomEvent('panora:products-changed'));window.panoraPricing?.notifyRetail(p.id,Number(p.basePrice??0));window.dispatchEvent(new CustomEvent('panora:retail-cloud-committed',{detail:{productId:p.id,price:Number(p.basePrice??0)}}));if(isNew){recipes[id]=[];store('panora-recipes',recipes);restaurants.forEach(r=>{r.prices??={};r.prices[id]=Number(p.basePrice||0)});cSave('panora-restaurants',restaurants);window.panoraCloud.queueRestaurants()}closeProductEditor();renderProductCards();renderAll();renderProductManagement()}catch(error){alert(`Не удалось сохранить хлеб: ${error.message||error}`)}finally{submit.disabled=false;submit.textContent='Сохранить карточку'}};
