(function(){
  ...
  function openProof(orderId){
    const note=deliveryNotes.find(n=>String(n.orderId)===String(orderId)),client=restaurant(note?.restaurantId);if(!note)return;closeProof();
    document.body.insertAdjacentHTML('beforeend',`<dialog id="offlineProofDialog" class="modal offline-proof-modal"><form class="dialog-card" id="offlineProofForm"><button type="button" class="modal-close" aria-label="Закрыть">×</button><p class="eyebrow">PANORA · ПОЛУЧЕНИЕ</p><h2>Подпись получателя</h2><p>${client?.name||'Партнёр'} проверяет поставку и расписывается на устройстве пекарни. Работает и онлайн, и без интернета.</p><label><span>Имя получателя</span><input name="receiver" required minlength="2" maxlength="120" autocomplete="name"></label><label><span>Подпись</span><canvas id="offlineSignature" width="700" height="260"></canvas></label><button type="button" class="clear-signature secondary">Очистить подпись</button><label class="proof-check"><input name="accepted" type="checkbox" required><span>Количество и состояние товара проверены.</span></label><button class="primary" type="submit">Сохранить получение по подписи</button></form></dialog>`);
