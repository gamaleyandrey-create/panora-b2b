(()=>{
  ...
  const refresh=async()=>{
    ...
    loading=true;
    try{
      const rows=await rest('restaurants?created_at=gte.2026-08-22T11%3A25%3A11Z&select=id,name,email,address,phone,whatsapp,telegram,extra_messengers,partner_type,language,active,updated_at,restaurant_prices(product_id,price,updated_at)&order=created_at.asc');
