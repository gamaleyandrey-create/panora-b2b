(()=>{
  ...
  const status=(text,error=false,detail='')=>{
    ...
    // Panora 10.99: keep the visible refresh line in the loading state for every
  ...
  function chooseConflictVersion(names){
    return new Promise(resolve=>{
      ...
      const modal=document.createElement('div');modal.id='panoraConflictChoice';modal.style.cssText='position:fixed;inset:0;z-index:10140;background:rgba(16,27,20,.58);display:grid;place-items:center;padding:20px';
  ...
  async function getAdminOperationalRevision(){
    ...
    const now=Date.now();
    if(adminOperationalRevisionCache&&now-adminOperationalRevisionAt<10140)return adminOperationalRevisionCache;
  ...
  async function getAdminReferenceRevision(){
    ...
    const now=Date.now();
    if(adminReferenceRevisionCache&&now-adminReferenceRevisionAt<10140)return adminReferenceRevisionCache;
  ...
  async function loadRestaurants(){
    ...
    if(rows?.length){
      ...
      // Panora 9.89 CLEAN BASE: Supabase is authoritative on first hydration.
  ...
  async function loadOrders(){
    ...
    loadingOrders=(async()=>{
      ...
      if(watermark&&!(fetched||[]).length){
        // Panora 10.99: a valid «no changes since watermark» response is still a
  ...
  async function repairMissingDeliveryNotes(remoteRowsHint=null,knownNotesHint=[]){
    if(repairingFinance)return repairingFinance;
    repairingFinance=(async()=>{
      // Panora 10.99: loadDeliveryNotes already owns a complete delivery-note read.
      // Reuse it during hydration instead of issuing a second request, while keeping
      // the no-argument repair path intact for manual/legacy callers.
      const remoteRows=Array.isArray(remoteRowsHint)?remoteRowsHint:await requestDeliveryNotes('');
      const knownNotes=Array.isArray(knownNotesHint)?knownNotesHint:[];
      ...
      for(const order of missing){
        const known=knownNotes.find(note=>String(note?.orderId||'')===String(order.id));
        // Restore only durable receipt evidence from the previous known note. Do not
        // treat an arbitrary stale local note as an unsent cloud delivery note.
        const preserved=known&&hasFinalReceiptEvidence(known)?known:null;
        const local=deliveryNotes.find(note=>note.orderId===order.id)||preserved||recoveredNote(order);
  ...
  const preserveFinalReceiptEvidence=(remoteNote,knownNote)=>{
    ...
    // Panora 10.32: receipt evidence is monotonic during ordinary cloud refreshes.
  ...
  async function loadDeliveryNotes(){
    ...
    loadingDeliveryNotes=(async()=>{
      ...
      // Panora 10.99: archive visibility is authoritative only after missing shipped
      // delivery notes have been reconciled. 10.99 unlocked the archive before this
      // repair and could therefore paint old completed orders as active or absent.
      // Reuse the already-fetched rows so this integrity gate does not add a second
      // delivery-note read; preserve only final receipt evidence from prior known data.
      await repairMissingDeliveryNotes(rows||[],[...knownNotes,...local]);
      ...
      // Stock durability is derived data and must not block the Orders screen.
      Promise.resolve().then(()=>syncB2BShipmentStockDurability())
        .catch(error=>console.warn('Panora delivery-note stock durability after hydration',error));
  ...
  async function requestPaymentsCompatible(deltaQuery=''){
    ...
    // Panora 10.32: the bakery must be able to read payments created by older
  ...
  async function loadPayments(){
    ...
    try{rows=await requestPaymentsCompatible(deltaQuery);window.panoraAdminPaymentsStale=false}
      ...
      if(!slow)throw error;
      // Panora 10.32: a slow payments endpoint must not block the whole mobile bakery.
    ...
    if(rows?.length){
      ...
      // Panora 9.89 CLEAN BASE: empty server means no payments. Do not upload
  ...
  async function savePaymentsNow(){
    ...
    if(!valid.length)return;
    status('Синхронизация…');
    const payload=valid.map(payment=>({id:payment.id,restaurant_id:payment.restaurantId,delivery_note_id:payment.deliveryNoteId||null,amount:Number(payment.amount),method:payment.method||'Не указан',note:payment.note||null,status:payment.status==='cancelled'?'cancelled':'confirmed',received_at:payment.receivedAt||`${localDate(payment.date)}T12:00:00Z`,confirmed_at:payment.confirmedAt||new Date().toISOString(),confirmed_by:session.user?.id||null,...(paymentDisputeColumnsSupported?{dispute_status:payment.disputeStatus||'none',dispute_reason:payment.disputeReason||null,disputed_at:payment.disputedAt||null,dispute_deadline:payment.disputeDeadline||null}:{})}));
    try{await request('payments?on_conflict=id',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=minimal'},body:JSON.stringify(payload)})}
    catch(error){
      if(!paymentDisputeColumnsSupported||!paymentDisputeSchemaError(error))throw error;
      paymentDisputeColumnsSupported=false;
      const legacyPayload=payload.map(({dispute_status,dispute_reason,disputed_at,dispute_deadline,...row})=>row);
      await request('payments?on_conflict=id',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=minimal'},body:JSON.stringify(legacyPayload)});
    }
    status('Облако ✓');
  }
  ...
  async function resolvePaymentDisputeAtomic(paymentId,decision){
    ...
    let rows;
    try{rows=await request(`payments?id=eq.${encodeURIComponent(paymentId)}`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify(paymentDisputeColumnsSupported?patch:{status:patch.status,updated_at:patch.updated_at})})}
    catch(error){
      if(!paymentDisputeColumnsSupported||!paymentDisputeSchemaError(error))throw error;
      paymentDisputeColumnsSupported=false;
      rows=await request(`payments?id=eq.${encodeURIComponent(paymentId)}`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify({status:patch.status,updated_at:patch.updated_at})});
    }
    const row=Array.isArray(rows)?rows[0]:rows;
...
async function ensureB2BReturnCreditPayments(){
  ...
  for(const note of deliveryNotes){
    ...
    for(const row of rows){
      ...
      payload.push({id:movementId,restaurant_id:note.restaurantId,delivery_note_id:note.id,amount:Number(row.gross||0),method:'Возврат товара',note:`${b2bReturnCreditPaymentMarker(movementId)} Возврат товара по DN-${String(note.number||'').padStart(4,'0')}`,status:'confirmed',received_at:row.movement?.createdAt||`${String(row.movement?.date||note.date||localDate(new Date().toISOString()))}T12:00:00Z`,confirmed_at:row.movement?.createdAt||new Date().toISOString(),confirmed_by:session.user?.id||null,...(paymentDisputeColumnsSupported?{dispute_status:'none'}:{})});
    }
  }
...
function financeTimeline(restaurantId){
  ...
  const paymentEvents=payments.filter(payment=>payment.restaurantId===restaurantId&&!isB2BReturnCreditPayment(payment)).flatMap(payment=>{
    ...
    const received={
      ...
      kind:'payment',
      sequence:paidAtShipment?Number(linkedNote.number||0)*2+1:1014000,
      payment,
      amount,
...
window.panoraRecalculateBalances=recalculateBalances;
  ...
  const isSafeExplicitPlanMove=(local,remote,{allowRepeat=false}={})=>{
    // Panora 10.99: the automatic migration remains one-shot, but an explicit
  ...
  async function refreshPlansManual(reason='manual-refresh'){
    ...
    // Panora 10.99: a manual/foreground refresh must not pull the cloud snapshot over
  ...
  async function refreshDeliveryReceipts(){
    if(!ready||!navigator.onLine||typeof deliveryNotes==='undefined')return false;
    const rows=await request('delivery_notes?select=id,order_id,customer_confirmed_at,customer_receiver,offline_received_at,offline_receiver,offline_signature');
    let changed=false;
    for(const row of rows||[]){
      ...
      note.customerConfirmedAt=nextConfirmed;
      note.customerReceiver=nextReceiver;
      note.offlineProof=nextProof;
      changed=true;
    }
    if(changed){
      cacheDeliveryNotesLocal();
      recalculateBalances();
      queueAdminCommerceRender();
      window.dispatchEvent(new CustomEvent('panora:delivery-receipt-updated',{detail:{source:'cloud-refresh'}}));
    }
    return changed;
  }
  ...
  const setAdminGlobalRefreshState=(state='idle')=>{
    ...
    // Panora 10.99: Refresh is never natively disabled. iOS/desktop must always be able
  ...
  async function settleAdminActiveView(view=adminActiveView()){
    // Panora 10.99: the global line may say «Данные актуальны» only after the
  ...
  async function refreshBreadStockData(){
    ...
    await loadOrders();
    await loadDeliveryNotes();
    try{await window.panoraFinishedStockPersistence?.loadCloud?.()}catch(error){console.warn('Panora finished stock cloud refresh',error)}
    if(typeof renderStock==='function')renderStock();
    window.dispatchEvent(new CustomEvent('panora:bread-stock-refreshed',{detail:{source:'cloud'}}));
    return true;
  }
  async function refreshAdminOrdersOnDemand(reason='manual'){
    if(!ready||!navigator.onLine)return false;
    const button=document.querySelector('#refreshOrdersCloud');
    if(button){button.disabled=true;button.dataset.loading='1';button.textContent='Обновляем…'}
    try{
      ...
      if(saveState?.dataset?.syncState==='error'&&/(заказ|наклад|подпис|получен|обновлен)/i.test(String(saveState.textContent||'')+' '+String(saveState.title||''))&&!pendingCount()&&!conflictCount())status('Облако ✓');
      window.dispatchEvent(new CustomEvent('panora:admin-orders-refreshed',{detail:{reason,changed:Boolean(changed?.changed)}}));
      return true;
    }finally{
  ...
  const refreshAdminCommerceOnWake=reason=>{
    ...
    adminWakeRefreshPromise=(async()=>{
      // Panora 10.99: foreground refresh uses the same safe calendar reconciliation
