(()=>{
  ...
  const productRow=(p,{includeTechCard=false}={})=>({id:p.id,name_ru:p.names?.ru||p.id,name_en:p.names?.en||p.names?.ru||p.id,name_es:p.names?.es||p.names?.ru||p.id,description_ru:p.descriptions?.ru||'',description_en:p.descriptions?.en||'',description_es:p.descriptions?.es||'',weight_g:Number(p.weight||750),base_price:Number(p.basePrice||0),wholesale_min_qty:Math.max(1,Number(p.wholesaleMinQty||12)),image_url:p.image||null,gallery_urls:Array.isArray(p.gallery)?p.gallery.filter(Boolean).slice(0,6):[],active:p.active!==false,storefront_visible:p.storefrontVisible!==false,...(includeTechCard?{tech_card:p.techCard||{}}:{}),updated_at:new Date().toISOString()});
  const rowProduct=(row,local)=>({id:row.id,builtIn:['plain','pumpkin'].includes(row.id),active:row.active,storefrontVisible:row.storefront_visible!==false,weight:Number(row.weight_g),basePrice:Number(row.base_price),wholesaleMinQty:Math.max(1,Number(row.wholesale_min_qty||local?.wholesaleMinQty||12)),image:row.image_url||local?.image||'icon.svg',gallery:Array.isArray(row.gallery_urls)?row.gallery_urls:(Array.isArray(local?.gallery)?local.gallery:[]),techCard:row.tech_card||{},techCardRevision:Number(row.tech_card_revision||0),names:{ru:row.name_ru,en:row.name_en,es:row.name_es},descriptions:{ru:row.description_ru||'',en:row.description_en||'',es:row.description_es||''}});
  ...
  const comparableProduct=p=>canonicalValue({id:String(p.id),active:p.active!==false,storefrontVisible:p.storefrontVisible!==false,weight:Number(p.weight),basePrice:Number(p.basePrice),wholesaleMinQty:Math.max(1,Number(p.wholesaleMinQty||12)),image:p.image||'icon.svg',gallery:Array.isArray(p.gallery)?p.gallery:[],techCard:p.techCard||{},techCardRevision:Number(p.techCardRevision||0),names:p.names||{},descriptions:p.descriptions||{}});
