(()=>{
  ...
  async function loadRestaurants(){
    ...
    if(rows?.length){
      ...
      // Panora 9.80 CLEAN BASE: Supabase is authoritative on first hydration.
  ...
  async function loadDeliveryNotes(){
    ...
    // Panora 9.80 CLEAN BASE: an arbitrary old local cache is never treated as
  ...
  async function loadPayments(){
    ...
    if(rows?.length){
      ...
      // Panora 9.80 CLEAN BASE: empty server means no payments. Do not upload
