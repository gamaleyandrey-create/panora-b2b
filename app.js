...
I18N.ru.hero.twice='По календарю';I18N.ru.hero.week='пекарни';
I18N.en.hero.twice='Bakery';I18N.en.hero.week='calendar';
I18N.es.hero.twice='Calendario';I18N.es.hero.week='de la panadería';
...
try{
 ...
 if(managedProducts.length){
  ...
  PRODUCTS.splice(0,PRODUCTS.length,...combined.filter(p=>p.active!==false).map(p=>{const base=builtIns.find(x=>x.id===p.id)||p;const retailPrice=Number(p.basePrice??p.retailPrice??p.price??0),wholesaleMinQty=Math.max(1,Number(p.wholesaleMinQty||8));return{id:p.id,category:String(p.category||'Хлеб'),price:retailPrice,retailPrice,wholesalePrice:retailPrice,wholesaleMinQty,pieces:wholesaleMinQty,weight:Number(p.weight||base.weight||750),image:(p.image&&!/(^|\/)icon\.svg(?:[?#]|$)/i.test(String(p.image))?p.image:(base.image||p.image||'icon.svg')),gallery:Array.isArray(p.gallery)?p.gallery:[],bg:base.bg||'#e9dfca',text:p.text||{ru:[p.names?.ru||p.id,p.descriptions?.ru||'',p.names?.ru||p.id],en:[p.names?.en||p.names?.ru||p.id,p.descriptions?.en||p.descriptions?.ru||'',p.names?.en||p.names?.ru||p.id],es:[p.names?.es||p.names?.ru||p.id,p.descriptions?.es||p.descriptions?.ru||'',p.names?.es||p.names?.ru||p.id]}}}));
 }
}catch(error){console.warn('Product cards could not be loaded',error)}
const MIN_PIECES=1,MAX_PIECES_PER_PRODUCT=200,PANORA_BAKERY_TIME_ZONE='Europe/Madrid';let SHOW_PRICES=true;
...
function applyLanguage(){document.documentElement.lang=lang;document.title=tr('title');if($('#languageSelect'))$('#languageSelect').value=lang;document.querySelectorAll('[data-i18n]').forEach(el=>el.textContent=tr(el.dataset.i18n));document.querySelectorAll('[data-i18n-html]').forEach(el=>el.innerHTML=tr(el.dataset.i18nHtml));document.querySelectorAll('[data-i18n-placeholder]').forEach(el=>el.placeholder=tr(el.dataset.i18nPlaceholder));const fulfillment=$('#fulfillmentSelect');if(fulfillment&&fulfillment.tagName==='SELECT')fulfillment.innerHTML=`<option value="delivery">${tr('checkout.deliveryOption')}</option><option value="pickup">${tr('checkout.pickupOption')}</option>`;else if(fulfillment)fulfillment.value='delivery';renderBakeDates();renderCategories();renderProducts();renderCart();toggleFulfillment()}
...
function availablePieces(id){const plan=productionPlans().find(p=>p.open!==false&&p.bakeDate===selectedBakeDate&&p.product===id);return plan?MAX_PIECES_PER_PRODUCT:0}
function catalogAvailablePieces(id){const plans=productionPlans(),now=new Date();if(!plans.length)return 0;const eligible=plans.filter(p=>p.open!==false&&p.product===id&&String(p.bakeDate||'')>=localToday()&&cutoffDate(p.cutoff)>now);return eligible.length?MAX_PIECES_PER_PRODUCT:0}
function partnerOrderQuantityLimit(id,currentQty=0){const live=catalogAvailablePieces(id);return account?MAX_PIECES_PER_PRODUCT:live}
function qtyControl(id,qty){const max=partnerOrderQuantityLimit(id,qty);if(max===0)return `<div class="sold-out"><strong>${lang==='ru'?'Нет ближайшей выпечки':lang==='es'?'No hay próximo horneado':'No upcoming bake'}</strong><span>${lang==='ru'?'Следите за расписанием':lang==='es'?'Consulta el calendario':'Check the schedule'}</span></div>`;const safeQty=Math.min(qty,max),options=Array.from({length:max+1},(_,n)=>`<option value="${n}" ${n===safeQty?'selected':''}>${piecesLabel(n)}</option>`).join('');return `<label class="piece-select"><select aria-label="${lang==='ru'?'Количество, шт.':lang==='es'?'Cantidad, uds.':'Quantity, pcs'}" data-qty-select="${id}">${options}</select></label>`}
...
function closePanels(){document.querySelectorAll('.drawer,.modal').forEach(p=>{p.classList.remove('open');p.setAttribute('aria-hidden','true')});$('#overlay').classList.remove('open');document.body.classList.remove('panora-workspace-checkout');document.body.style.overflow=''}
...
function toggleFulfillment(){const fulfillment=$('#fulfillmentSelect');const pickup=fulfillment?.value==='pickup';const address=$('#deliveryAddressLabel'),time=$('#deliveryTimeLabel');if(address)address.style.display=pickup?'none':'grid';if(time)time.style.display=pickup?'none':'grid';const input=address?.querySelector('input');if(input)input.required=!pickup}
function orderText(order){const lines=[tr('order.title'),`${tr('order.number')}: ${order.id}`,`${tr('order.restaurant')}: ${order.restaurant}`,`${tr('order.contact')}: ${order.contact}`,`${tr('order.phone')}: ${order.phone}`];if(order.email)lines.push(`${tr('order.email')}: ${order.email}`);lines.push(`${tr('order.bake')}: ${order.dateLabel}`);if(order.address)lines.push(`${tr('order.address')}: ${order.address}`);if(order.time)lines.push(`${tr('order.time')}: ${order.time}`);lines.push('',`${tr('order.items')}:`,...order.items.map(i=>`• ${pText(PRODUCTS.find(p=>p.id===i.id))[0]} — ${piecesLabel(i.quantityPieces)}`));if(SHOW_PRICES)lines.push(`${tr('order.total')}: ${money(order.total)}`);if(order.comment)lines.push('',order.comment);return lines.join('\n')}
...
if($('#languageSelect'))$('#languageSelect').onchange=e=>panoraSetLanguage(e.target.value);$('#cartButton').onclick=()=>openPanel($('#cartDrawer'));$('#checkoutButton').onclick=()=>{
  // Panora 9.89: partner-workspace orders skip the basket, so make sure the
  // final confirmation owns a fresh, selectable bake-date control before it opens.
  try{syncCartDeliveryDate()}catch{}
  closePanels();
  setTimeout(()=>{
    try{syncCartDeliveryDate()}catch{}
    const modal=$('#checkoutModal'),date=$('#deliveryDate');
    if(date){date.required=true;date.disabled=!date.options.length||!date.value&&date.options.length===1&&date.options[0].value===''}
    openPanel(modal);
    requestAnimationFrame(()=>{try{syncCartDeliveryDate()}catch{}});
  },180)
};$('#overlay').onclick=closePanels;document.querySelectorAll('[data-close]').forEach(b=>b.onclick=closePanels);const openPartnerCabinet=()=>{const modal=$('#profileModal');try{if(typeof window.panoraOpenPartnerCabinet==='function')return window.panoraOpenPartnerCabinet();renderAccountModal?.()}catch{}requestAnimationFrame(()=>openPanel(modal))};
...
if($('#fulfillmentSelect'))$('#fulfillmentSelect').onchange=toggleFulfillment;
