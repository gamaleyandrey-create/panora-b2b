...
(()=>{
  ...
  function rowHtml(pid,item,index,total){const unit=item.unit||'g',qty=numeric(item.qty),isBase=gramUnit(unit)&&flourName(item.name),percent=total&&unit!=='pcs'?display(qty/total*100):'';return `<div class="recipe-row recipe-percent-row" data-row-id="${esc(pid)}:${index}" data-index="${index}">
