...
(()=>{
  ...
  const setTechCardOpen=(productId,open)=>{
    const state=readTechCardDisclosure();
    state[String(productId)]=Boolean(open);
    localStorage.setItem(TECH_CARD_DISCLOSURE_KEY,JSON.stringify(state));
    return state[String(productId)];
  };
  const recipeArchived=productId=>Boolean((typeof recipeProduct==='function'?recipeProduct(productId):null)?.techCard?.recipeArchived);
  async function setRecipeArchivedConfirmed(productId,archived,button=null){
    const product=typeof recipeProduct==='function'?recipeProduct(productId):null;
    if(!product)throw new Error('Не удалось определить рецептуру');
    if(!navigator.onLine||!window.panoraCloud?.ready)throw new Error(L().lockOnline);
    const previous=Boolean(product.techCard?.recipeArchived);
    if(previous===Boolean(archived))return true;
    if(button){button.disabled=true;button.dataset.previousText=button.textContent;button.textContent=L().saving}
    try{
      if(!window.panoraCloud?.hasTechCardLock?.(productId))await window.panoraCloud.acquireTechCardLock(productId);
      const next={...(product.techCard||{}),recipeArchived:Boolean(archived)};
      const confirmed=await window.panoraCloud.saveProductTechCardConfirmed(productId,next);
      product.techCard={...confirmed};
      ...
      window.dispatchEvent(new CustomEvent('panora:products-changed',{detail:{source:archived?'recipe-archived':'recipe-restored',productId}}));
      professionalRender(true);
      return true;
    }catch(error){
      try{await window.panoraCloud?.releaseTechCardLock?.(productId)}catch{}
      throw error;
    }finally{
      if(button&&document.body.contains(button)){button.disabled=false;button.textContent=button.dataset.previousText||button.textContent}
    }
  }
  ...
  function refreshRowPrice(row){
    ...
    state.textContent=target.name?(price>0?(target.source?`${target.name} · ${L().priceOne}`:L().priceOne):L().priceMissing):(target.source?L().semiSource:L().ingredient);
    state.classList.toggle('missing',Boolean(target.name)&&price<=0);
  }
  function refreshSemiCost(row){
    const box=row.querySelector('[data-role="semiCost"]');
    if(!box)return;
    const enabled=Boolean(row.querySelector('[data-role="sourceEnabled"]')?.checked);
    box.hidden=!enabled;
    if(!enabled)return;
    const qty=Math.max(0,numeric(row.querySelector('[data-role="qty"]')?.value));
    const outputUnit=row.querySelector('[data-role="unit"]')?.value||'g';
    const sourceName=row.querySelector('[data-role="sourceName"]')?.value.trim()||'';
    const sourceUnit=row.querySelector('[data-role="sourceUnit"]')?.value||outputUnit;
    const yieldPct=Math.max(0,numeric(row.querySelector('[data-role="sourceYield"]')?.value));
    const sourcePrice=sourceName?ingredientPrice(readIngredientPrices(),sourceName,sourceUnit):0;
    const yieldFactor=yieldPct/100;
    const sourceFactor=(sourceUnit==='g'||sourceUnit==='ml')?1000:1;
    const outputFactor=(outputUnit==='g'||outputUnit==='ml')?1000:1;
    const sourceQty=yieldFactor>0?qty/yieldFactor:0;
    const recipeCost=sourcePrice>0&&sourceQty>0?sourceQty/sourceFactor*sourcePrice:0;
    const unitCost=sourcePrice>0&&yieldFactor>0?sourcePrice*outputFactor/sourceFactor/yieldFactor:0;
    const unitCostEl=box.querySelector('[data-semi-unit-cost]');
    const sourceQtyEl=box.querySelector('[data-semi-source-qty]');
    const recipeCostEl=box.querySelector('[data-semi-recipe-cost]');
    const stateEl=box.querySelector('[data-semi-cost-state]');
    if(unitCostEl)unitCostEl.textContent=unitCost>0?`${display(unitCost)} ${priceUnitLabel(outputUnit)}`:'—';
    if(sourceQtyEl)sourceQtyEl.textContent=sourceQty>0?`${display(sourceQty)} ${sourceUnit}`:'—';
    if(recipeCostEl)recipeCostEl.textContent=recipeCost>0?`${display(recipeCost)} €`:'—';
    if(stateEl){
      stateEl.textContent=sourceName&&yieldPct>0&&sourcePrice>0?`${sourceName} · ${display(sourcePrice)} ${priceUnitLabel(sourceUnit)} · ${display(yieldPct)}%`:L().semiCostMissing;
      stateEl.classList.toggle('missing',!(sourceName&&yieldPct>0&&sourcePrice>0));
    }
  }
  ...
  function updateCard(card,preservePercent=null){
    ...
    rows.forEach((row,index)=>{const semi=Boolean(row.querySelector('[data-role="sourceEnabled"]')?.checked),item={...(stored[index]||{}),name:row.querySelector('[data-role="name"]')?.value||'',unit:row.querySelector('[data-role="unit"]')?.value||'g',qty:numeric(row.querySelector('[data-role="qty"]')?.value),sourceIngredientName:semi?(row.querySelector('[data-role="sourceName"]')?.value.trim()||''):'',sourceUnit:row.querySelector('[data-role="sourceUnit"]')?.value||'g',sourceYieldPct:semi?Math.max(0,numeric(row.querySelector('[data-role="sourceYield"]')?.value)):0};if(item.qty>0){costable++;const result=rowCostPerRecipe(item,prices);if(result.priced){priced++;recipeCost+=result.cost}}});
    ...
    card.querySelectorAll('.recipe-percent-row').forEach(row=>{refreshRowPrice(row);refreshSemiCost(row)});
  }
  ...
  function rowHtml(pid,item,index,total){
    ...
    return `<div class="recipe-row recipe-percent-row${isSemi?' is-semi':''}" data-row-id="${esc(pid)}:${index}" data-index="${index}">
      <label class="recipe-field recipe-field-name"><small>${L().ingredient}</small><input data-role="name" data-draft-key="name" list="panora-raw-catalog" autocomplete="off" placeholder="${L().catalogHint}" value="${esc(item.name)}" aria-label="${L().ingredient}"><span class="recipe-catalog-state" data-catalog-state hidden></span></label>
      ...
      <section class="recipe-semi-settings">
        ...
        <div class="recipe-semi-fields" ${isSemi?'':'hidden'}>
          ...
          <div class="recipe-semi-cost" data-role="semiCost" ${isSemi?'':'hidden'}>
            <div class="recipe-semi-cost-state" data-semi-cost-state>${L().semiCostMissing}</div>
            <div><small>${L().semiUnitCost}</small><strong data-semi-unit-cost>—</strong><span>${L().semiCostFormula}</span></div>
            <div><small>${L().semiSourceNeed}</small><strong data-semi-source-qty>—</strong><span>${source?esc(source):L().semiSource}</span></div>
            <div><small>${L().semiRecipeCost}</small><strong data-semi-recipe-cost>—</strong><span>${L().semiPerBread}</span></div>
          </div>
          <small class="recipe-semi-example">${lang==='ru'?'Выход — это доля готового полуфабриката из исходного сырья. Например, при выходе 75% из 1 кг свежей тыквы получается 0,75 кг пюре.':lang==='es'?'El rendimiento es la proporción de producto terminado obtenida de la materia prima.':'Yield is the finished semi-product obtained from the raw material.'}</small>
        </div>
      </section>
    </div>`
  }
  function professionalRender(force=false){
    ...
    root.querySelectorAll('.recipe-card').forEach(card=>{
      const pid=card.dataset.recipeCard;
      if(recipeArchived(pid)){archived.push(pid);card.remove()}
    });
    if(archiveCount)archiveCount.textContent=String(archived.length);
    const archiveTitle=document.querySelector('[data-recipe-archive-title]'),archiveUnit=document.querySelector('[data-recipe-archive-unit]');
    if(archiveTitle)archiveTitle.textContent=L().archiveTitle;if(archiveUnit)archiveUnit.textContent=lang==='ru'?'шт.':lang==='es'?'uds.':'pcs';
    ...
    root.querySelectorAll('.recipe-card').forEach(card=>{
      ...
      techDetails?.addEventListener('toggle',()=>setTechCardOpen(card.dataset.recipeCard,techDetails.open));
      ...
      card.querySelectorAll('[data-role="name"]').forEach(input=>{
        const matchCatalog=()=>{const raw=(window.panoraRawStock?.catalog?.()||[]),value=String(input.value||'').trim().toLocaleLowerCase('ru-RU').replace(/ё/g,'е'),match=raw.find(item=>String(item.name||'').trim().toLocaleLowerCase('ru-RU').replace(/ё/g,'е')===value),row=input.closest('.recipe-percent-row'),state=row?.querySelector('[data-catalog-state]');if(match){const unit=row?.querySelector('[data-role="unit"]');if(unit&&unit.value!==match.unit)unit.value=match.unit;if(state)state.hidden=true;updateDraft();updateCard(card)}else if(value&&state){state.hidden=false;state.innerHTML=`${L().catalogMissing} · <button type="button" data-catalog-add>${L().catalogAdd}</button>`;state.querySelector('[data-catalog-add]')?.addEventListener('click',event=>{event.preventDefault();window.panoraRawStock?.openNew?.({name:input.value,unit:row?.querySelector('[data-role="unit"]')?.value||'g'})})}else if(state)state.hidden=true};
        input.addEventListener('change',matchCatalog);input.addEventListener('blur',()=>setTimeout(matchCatalog,80));
      });
    ...
    root.querySelectorAll('[data-archive-recipe]').forEach(button=>button.onclick=async()=>{const pid=button.dataset.archiveRecipe;if(!confirm(L().archiveConfirm))return;try{await setRecipeArchivedConfirmed(pid,true,button)}catch(error){alert(`${L().archiveFailed}: ${error?.message||error}`)}});
  ...
  window.addEventListener('panora:raw-catalog-changed',()=>{const list=document.querySelector('#panora-raw-catalog');if(!list)return;const raw=window.panoraRawStock?.catalog?.()||[];list.innerHTML=raw.slice().sort((a,b)=>String(a.name).localeCompare(String(b.name),'ru')).map(row=>`<option value="${esc(row.name)}" label="${esc(row.unit==='g'?'кг / г':row.unit==='ml'?'л / мл':'шт.')}"></option>`).join('')});
})();
