(function(){
 ...
 renderPurchase=function(){
  ...
  $$('[data-ingredient-price]').forEach(input=>input.onchange=()=>{priceMap[input.dataset.ingredientPrice]=Math.max(0,Number(input.value||0));saveCosts(priceMap);window.dispatchEvent(new CustomEvent('panora:ingredient-costs-changed'));renderPurchase()});
