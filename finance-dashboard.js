(()=>{
  ...
  // Panora 7.11: cloud event timestamps are UTC, but Finance periods are bakery-local calendar days.
  ...
  const inPeriod=date=>{const d=localEventDay(date);return (!from.value||d>=from.value)&&(!to.value||d<=to.value)};
  const periodLabel=()=>{
  ...
  function calculate(){
    ...
    const b2bShipmentOrderId=movement=>{const direct=String(movement?.orderId||'');if(direct)return direct;const match=String(movement?.note||'').match(/\[panora:b2b-shipment:([^:\]]+):[^:\]]+:\d+\]/);return match?String(match[1]):''};
    ...
    const stampLocalDay=localEventDay;
    ...
    const historicalUnitRawCostAt=(product,date,stamp)=>{
      const day=String(date||'').slice(0,10),cutoff=economicEventStamp(day,stamp);let fallbackRecipe=null;
      for(let i=costBakeCompletions.length-1;i>=0;i--){
        const completion=costBakeCompletions[i],completionDay=String(completion?.date||'').slice(0,10);if(day&&completionDay>day)continue;
        if(day&&completionDay===day&&String(economicEventStamp(completionDay,completion?.createdAt))>String(cutoff))continue;
        const item=(Array.isArray(completion?.items)?completion.items:[]).find(row=>String(row?.product||'')===String(product));if(!item)continue;
        const snapshot=item.costSnapshot,unit=Number(snapshot?.unitRawCost);if(snapshot?.complete===true&&Number.isFinite(unit)&&unit>=0)return unit;
        if(!fallbackRecipe&&Array.isArray(item.recipeSnapshot)&&item.recipeSnapshot.length)fallbackRecipe=item.recipeSnapshot;
      }
      return fallbackRecipe?recipeUnitRawCost(fallbackRecipe,product):unitRawCost(product);
    };
    ...
    // Panora 7.07: preserve the real same-day bake completion time so an inventory checkpoint
    // earlier that day stays earlier than a later bake in both physical stock and Finance FIFO.
    costBakeCompletions.forEach(completion=>(Array.isArray(completion?.items)?completion.items:[]).forEach((item,index)=>{const product=String(item?.product||''),good=Math.max(0,Number(item?.good??(Number(item?.produced||0)-Number(item?.waste||0))));if(!product||good<=0)return;const legacyManual=completion?.source==='legacy_inferred'?Number(manualProducedByDayProduct.get(`${String(completion?.date||'').slice(0,10)}\u0000${product}`)||0):0,qty=Math.max(0,good-legacyManual);if(qty<=0)return;costEvents.push({kind:'add',product,qty,unitCost:frozenBakeUnitCost(item,product,completion.date),source:`bake:${completion.id||completion.date}:${index}`,key:costEventKey(completion.date,economicEventStamp(completion.date,completion.createdAt),0,`bake:${completion.id||completion.date}:${index}`)})}));
    const noteOrderIds=new Set(allNotes.map(note=>String(note?.orderId||'')).filter(Boolean));
    allFinishedMovements.forEach((movement,index)=>{const product=String(movement?.product||''),type=String(movement?.type||''),qty=Math.max(0,Math.abs(Number(movement?.quantity||0)));if(!product||qty<=0)return;const shipmentOrderId=b2bShipmentOrderId(movement);if(shipmentOrderId&&noteOrderIds.has(shipmentOrderId))return;const date=String(movement?.date||'').slice(0,10),stamp=movement?.occurredAt||movement?.createdAt||`${date}T12:00:00.000Z`,id=String(movement?.id||`manual:${index}`),target=stockInventoryTarget(movement),fallback=historicalUnitRawCostAt(product,date,stamp);if(target!==null){costEvents.push({kind:'inventory',product,target,fallback,key:costEventKey(date,stamp,45,id),id});return}if(['produced','initial_balance','correction_plus'].includes(type)){costEvents.push({kind:'add',product,qty,unitCost:fallback,source:`stock:${id}`,key:costEventKey(date,stamp,10,id)});return}if(type==='returned'){costEvents.push({kind:'return',product,qty,movement,b2bNoteId:b2bReturnNoteId(movement),retailOrderId:retailReturnOrderId(movement),fallback,key:costEventKey(date,stamp,35,id),id});return}if(type==='written_off'){costEvents.push({kind:'writeoff',product,qty,fallback,key:costEventKey(date,stamp,40,id),id});return}if(['correction_minus','shipped','retail_sold'].includes(type)){costEvents.push({kind:'consume',product,qty,fallback,key:costEventKey(date,stamp,30,id),id})}});
    allNotes.forEach(note=>{const grouped=new Map();(Array.isArray(note?.items)?note.items:[]).forEach(item=>{const product=String(item?.product||''),qty=Math.max(0,Number(item?.quantity||0));if(product&&qty)grouped.set(product,(grouped.get(product)||0)+qty)});const saleStamp=economicEventStamp(note?.date,note?.createdAt,note?.customerConfirmedAt,note?.offlineProof?.receivedAt);grouped.forEach((qty,product)=>costEvents.push({kind:'b2b-sale',product,qty,note,fallback:historicalUnitRawCostAt(product,note?.date,saleStamp),key:costEventKey(note?.date,saleStamp,20,`b2b:${note?.id}:${product}`)}))});
    allRetailForCost.filter(order=>String(order?.status||'')==='completed').forEach(order=>{const grouped=new Map();(Array.isArray(order?.items)?order.items:[]).forEach(item=>{const product=String(item?.product||''),qty=Math.max(0,Number(item?.quantity||0));if(product&&qty)grouped.set(product,(grouped.get(product)||0)+qty)});const saleDay=localEventDay(order?.completedAt||order?.pickupDate||order?.createdAt||''),saleStamp=economicEventStamp(saleDay,order?.completedAt,order?.updatedAt,order?.createdAt);grouped.forEach((qty,product)=>costEvents.push({kind:'retail-sale',product,qty,order,fallback:historicalUnitRawCostAt(product,saleDay,saleStamp),key:costEventKey(saleDay,saleStamp,25,`retail:${order?.id}:${product}`)}))});
    costEvents.sort((a,b)=>String(a.key).localeCompare(String(b.key)));
    // Panora 7.14: Finance FIFO must enforce the same sold-quantity ceiling as the
    // physical retail stock ledger. P&L already capped repeated returns in 7.13, but
    // the cost-layer engine still accepted every duplicate movement and could feed a
    // later sale/write-off with phantom returned COGS.
    const acceptedRetailReturnCostQty=new Map();
    costEvents.forEach(event=>{if(event.kind==='add'){addLayer(event.product,event.qty,event.unitCost,event.source);return}if(event.kind==='inventory'){const current=layerQty(event.product);if(current>event.target+1e-9)consumeLayers(event.product,current-event.target,event.fallback);else if(current+1e-9<event.target)addLayer(event.product,event.target-current,event.fallback,`inventory:${event.id}`);return}if(event.kind==='b2b-sale'){const allocation=consumeLayers(event.product,event.qty,event.fallback);b2bSaleCost.set(`${String(event.note?.id||'')}\u0000${event.product}`,{...allocation,qty:event.qty});return}if(event.kind==='retail-sale'){const allocation=consumeLayers(event.product,event.qty,event.fallback);retailSaleCost.set(`${String(event.order?.id||'')}\u0000${event.product}`,{...allocation,qty:event.qty});return}if(event.kind==='return'){let profile=null,acceptedQty=event.qty;if(event.b2bNoteId)profile=b2bSaleCost.get(`${event.b2bNoteId}\u0000${event.product}`);else if(event.retailOrderId){profile=retailSaleCost.get(`${event.retailOrderId}\u0000${event.product}`);if(profile&&Number(profile.qty)>0){const key=`${event.retailOrderId}\u0000${event.product}`,used=Math.max(0,Number(acceptedRetailReturnCostQty.get(key)||0)),capacity=Math.max(0,Number(profile.qty||0)-used);acceptedQty=Math.min(Math.max(0,Number(event.qty||0)),capacity);acceptedRetailReturnCostQty.set(key,used+acceptedQty)}}const unit=profile&&Number(profile.qty)>0?Number(profile.cost||0)/Number(profile.qty):event.fallback,cost=unit*acceptedQty;returnMovementCost.set(event.id,cost);addLayer(event.product,acceptedQty,unit,`return:${event.id}`);return}if(event.kind==='writeoff'){const allocation=consumeLayers(event.product,event.qty,event.fallback);writeOffCost.set(event.id,allocation.cost);return}if(event.kind==='consume')consumeLayers(event.product,event.qty,event.fallback)});
    ...
    retailRefunded.forEach(order=>{
      const x=retailParts(order);retailRefundsGross+=x.orderGross;
      ...
      x.items.forEach(item=>{
        const qty=Math.max(0,Number(item.quantity||0));if(!qty)return;
        const itemGrossValue=qty*Math.max(0,Number(item.unitPrice||0)),itemNet=x.breadNet*(itemGrossValue/grossBasis);
        const p=productMap.get(item.product)||{product:item.product,pieces:0,revenue:0,cogs:0,b2bPieces:0,retailPieces:0,b2bRevenue:0,retailRevenue:0};
  ...
  // Panora 10.41 — raw-material purchase entry from Raw Materials stock.
