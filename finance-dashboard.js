(()=>{
  ...
  function calculate(){
    ...
    allRetailForCost.filter(order=>String(order?.status||'')==='completed').forEach(order=>{const grouped=new Map();(Array.isArray(order?.items)?order.items:[]).forEach(item=>{const product=String(item?.product||''),qty=Math.max(0,Number(item?.quantity||0));if(product&&qty)grouped.set(product,(grouped.get(product)||0)+qty)});const saleDay=String(order?.completedAt||order?.pickupDate||order?.createdAt||'').slice(0,10),saleStamp=order?.completedAt||order?.updatedAt||order?.createdAt||`${saleDay}T12:00:00.000Z`;grouped.forEach((qty,product)=>costEvents.push({kind:'retail-sale',product,qty,order,fallback:historicalUnitRawCostAt(product,saleDay,saleStamp),key:costEventKey(saleDay,saleStamp,25,`retail:${order?.id}:${product}`)}))});
