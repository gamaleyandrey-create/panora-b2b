...
(()=>{
  ...
  async function fetchPartnerDeliveryNotes(rid){
    ...
    const preferred=`delivery_notes?restaurant_id=eq.${encoded}&select=id,note_number,order_id,restaurant_id,delivered_at,payment_due_date,total,trays_delivered,trays_returned,tray_balance_after,customer_trays_received,customer_trays_returned,qr_token,customer_confirmed_at,customer_receiver,offline_received_at,offline_receiver,lot_trace`;
  ...
  async function loadAll(force=false){
    ...
    loadPromise=(async()=>{
      ...
      if(restaurantRows[0].active===false){
        const inactiveMessage=labels('Профиль партнёра архивирован пекарней. Новые заказы недоступны.','The partner profile has been archived by the bakery. New orders are unavailable.','El perfil del socio ha sido archivado por la panadería. No se pueden realizar nuevos pedidos.');
      ...
      // Panora 9.57: the mobile workspace may already be open while the
  ...
  const partnerNoteSignature=list=>JSON.stringify((list||[]).map(note=>({
    ...
    customerConfirmedAt:String(note?.customerConfirmedAt||''),customerReceiver:String(note?.customerReceiver||''),lotTrace:JSON.stringify(note?.lotTrace||[])
  ...
  async function refreshPartnerFinance({notes:wantNotes=true,payments:wantPayments=true}={}){
    ...
    partnerFinanceLoading=(async()=>{
      ...
      const [notes,paymentRows]=await Promise.all([
        wantNotes?fetchPartnerDeliveryNotes(account.id):Promise.resolve(null),
        wantPayments?fetchPartnerPayments(account.id,paymentDelta):Promise.resolve(null)
      ]);
  ...
  function openMobilePartnerCabinetAfterAuth(delay=0){
    window.setTimeout(()=>{
      ...
      if(mobile){
        // Panora 9.57: portal-cloud loads before restaurant-workspace.js.
  ...
  form?.addEventListener('submit',async event=>{
    ...
    // Panora 9.89: mobile partner checkout may be opened directly from the
