(()=>{
  ...
  let refreshing=null;
  let cloudState={type:'ok',text:'Соединение установлено'};
  function setCloudState(type,text){cloudState={type,text};try{localStorage.setItem('panora-restaurant-cloud-state',JSON.stringify(cloudState))}catch{}decorateCloudState()}
  function decorateCloudState(){
    if(!account)return;const modal=document.querySelector('#profileModal'),anchor=modal?.querySelector('.account-section');if(!anchor)return;let box=modal.querySelector('#restaurantCloudState');if(!box){box=document.createElement('section');box.id='restaurantCloudState';box.className='account-section';anchor.before(box)}
    box.innerHTML=`<h3>${lang==='ru'?'Облако':lang==='es'?'Nube':'Cloud'}</h3><p style="color:${cloudState.type==='error'?'#a5443c':'#42684d'};overflow-wrap:anywhere">${cloudState.text}</p>${cloudState.type==='error'?`<button type="button" class="button button-ghost full" id="retryCloudOrder">${lang==='ru'?'Повторить отправку':lang==='es'?'Reintentar envío':'Retry sending'}</button>`:''}`;const retry=box.querySelector('#retryCloudOrder');if(retry)retry.onclick=retryPending
  }
  ...
  async function hydrate(){
    ...
    account=own;localStorage.setItem('panora-account-id',own.id);applyAccount();if(typeof refreshRestaurantData==='function')refreshRestaurantData();if(!['error','sending'].includes(cloudState.type))setCloudState('ok',lang==='ru'?'Соединение установлено':lang==='es'?'Conexión establecida':'Connected');return true;
  }
  async function uploadOrder(order){
    if(!session?.access_token||!order||uploading)return;
    uploading=true;setCloudState('sending',lang==='ru'?'Отправляем заказ…':lang==='es'?'Enviando pedido…':'Sending order…');
    try{
      ...
      await hydrate();setCloudState('ok',lang==='ru'?`Заказ PN-${String(created.order_number).padStart(4,'0')} отправлен пекарне`:lang==='es'?'Pedido enviado a la panadería':'Order sent to the bakery');showToast(cloudState.text);
    }catch(error){console.error('Panora restaurant cloud order',error);setCloudState('error',(lang==='ru'?'Заказ не отправлен: ':'Cloud order failed: ')+error.message);showToast(cloudState.text)}finally{uploading=false}
  }
