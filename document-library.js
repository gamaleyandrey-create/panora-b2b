(() => {
  ...
  window.openPanoraDocumentLibrary = (value, options = {}) => {
    ...
    const defaults = {
      ...
      documentNumber:saved.documentNumber || `F-${new Date().getFullYear()}-${String(note.number || 1).padStart(4,"0")}`,
    ...
    dialog.innerHTML=`<div class="document-library-shell"><button class="document-library-x" aria-label="${esc(t("close"))}">×</button><span class="document-library-kicker">PANORA</span><h2>${esc(t("title"))}</h2><p>${esc(t("hint"))}</p><div class="document-library-grid"><button data-doc="panora"><strong>${esc(t("panora"))}</strong><small>${esc(t("panoraHint"))}</small></button><button data-doc="albaran"><strong>${esc(t("albaran"))}</strong><small>${esc(t("albaranHint"))}</small></button><button data-doc="factura"><strong>${esc(t("factura"))}</strong><small>${esc(t("facturaHint"))}</small></button></div><form class="document-factura-form" hidden><div class="document-party"><h3>${esc(t("seller"))}</h3><label>${esc(t("legal"))}<input name="sellerLegalName" value="${esc(defaults.sellerLegalName)}"></label><label>${esc(t("tax"))}<input name="sellerTaxId" value="${esc(defaults.sellerTaxId)}"></label><label>${esc(t("address"))}<input name="sellerAddress" value="${esc(defaults.sellerAddress)}"></label></div><div class="document-party"><h3>${esc(t("buyer"))}</h3><label>${esc(t("legal"))}<input name="buyerLegalName" value="${esc(defaults.buyerLegalName)}"></label><label>${esc(t("tax"))}<input name="buyerTaxId" value="${esc(defaults.buyerTaxId)}"></label><label>${esc(t("address"))}<input name="buyerAddress" value="${esc(defaults.buyerAddress)}"></label></div><label>${esc(t("number"))}<input name="documentNumber" value="${esc(defaults.documentNumber)}"></label><label>${esc(t("vat"))}<select name="ivaRate">${[0,4,10,21].map(v=>`<option value="${v}"${Number(defaults.ivaRate)===v?" selected":""}>${v}%</option>`).join("")}</select></label><label class="document-check"><input type="checkbox" name="pricesIncludeTax"${defaults.pricesIncludeTax?" checked":""}> ${esc(t("included"))}</label><p class="document-form-error" role="alert"></p><button class="document-create" type="submit">${esc(t("open"))}</button></form></div>`;
    ...
    dialog.querySelector('[data-doc="albaran"]').onclick=()=>{ close(); setTimeout(()=>window.openAccountingInvoice?.(note,{...options,variant:"albaran",documentData:defaults}),0); };
    dialog.querySelector('[data-doc="factura"]').onclick=()=>{dialog.querySelector(".document-factura-form").hidden=false;};
    dialog.querySelector("form").onsubmit=e=>{e.preventDefault(); const data=Object.fromEntries(new FormData(e.currentTarget)); data.pricesIncludeTax=e.currentTarget.pricesIncludeTax.checked; data.ivaRate=Number(data.ivaRate); const required=["sellerLegalName","sellerTaxId","sellerAddress","buyerLegalName","buyerTaxId","buyerAddress","documentNumber"]; if(required.some(k=>!String(data[k]||"").trim())){e.currentTarget.querySelector(".document-form-error").textContent=t("required");return;} localStorage.setItem(key(note),JSON.stringify(data)); close(); setTimeout(()=>window.openAccountingInvoice?.(note,{...options,variant:"factura",documentData:data}),0);};
