...
function restaurantCancelOrder(id) {
  ...
  const number = `PN-${String(order.number).padStart(4, "0")}`,
    items = order.items
      .map(
        (i) => `${portalProduct(i.product)} — ${i.quantity} ${portalPieces()}`,
      )
      .join("\n"),
    question =
      lang === "ru"
  ...
  const released = order.items
      .map((i) => `${portalProduct(i.product)} ${i.quantity}`)
      .join(", "),
    message =
      lang === "ru"
