...
assert.equal(manifest.name,'Panora');assert.equal(manifest.display,'standalone');assert.match(manifest.start_url,/build=3356/);assert.ok(manifest.icons.some(x=>x.src==='icon.svg'));
assert.match(sw,/panora-v3356/);assert.match(sw,/skipWaiting/);assert.match(sw,/clients\.claim/);assert.match(sw,/PANORA_SKIP_WAITING/);
...
for(const h of [partner,admin]){assert.match(h,/manifest\.webmanifest\?v=3340/);assert.match(h,/pwa-stable\.css\?v=3340/);assert.match(h,/pwa-stable\.js\?v=3356/);assert.match(h,/stable-health\.js\?v=3340/)}
