...
function renderBakeCalendar(){
 ...
 for(let day=1;day<=days;day++){const d=new Date(year,month,day),date=iso(d),dayLabel=new Intl.DateTimeFormat(locales[l],{day:'numeric',month:'short'}).format(d),entries=plans.filter(p=>p.bakeDate===date),details=entries.map(p=>`<span><strong>${productName(p.product)}</strong><i>${p.planned} шт. · ${p.ordered||0} заказано</i></span>`).join('');cells.push(`<button class="calendar-day ${date===today?'today':''} ${entries.length?'has-bake':''}" data-calendar-date="${date}" aria-label="${entries.length?'Открыть выпечку':'Запланировать выпечку'} ${dayLabel}"><em>${dayLabel}</em>${details}</button>`)}
