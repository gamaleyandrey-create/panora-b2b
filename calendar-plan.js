...
function renderBakeCalendar(){
 ...
 const monthPrefix=`${year}-${String(month+1).padStart(2,'0')}`,monthPlans=plans.filter(p=>p.bakeDate.startsWith(monthPrefix)),planned=monthPlans.reduce((sum,p)=>sum+Number(p.planned||0),0),ordered=monthPlans.reduce((sum,p)=>sum+Number(p.ordered||0),0);
 ...
 document.querySelectorAll('[data-calendar-date]').forEach(b=>b.onclick=()=>{const date=b.dataset.calendarDate;weekStart=startOfWeek(new Date(`${date}T12:00:00`));renderPlan();document.querySelector('#addPlan').click();const f=document.querySelector('#planForm');f.bakeDate.value=date;setDefaultPlanDates(f,date)})
}
