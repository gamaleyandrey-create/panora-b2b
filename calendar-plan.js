...
function alignMobileCalendar(today,shownPrefix){
 ...
 requestAnimationFrame(()=>{
   ...
   if(!target)return;
   // Panora 10.88: mobile calendar now fits the viewport instead of using a wide
...
function renderBakeCalendar(){
 const planRows=calendarPlanRows();
...
['panora:plans-updated','panora:plan-saved','panora:plans-conflict','panora:admin-startup-recovered'].forEach(name=>window.addEventListener(name,()=>renderBakeCalendar()));
