...
(() => {
    ...
    return `<section class="rw-profile-page"><aside class="rw-profile rw-profile-summary" aria-label="${esc(account.name)}">
      ...
      <section class="rw-password-panel" aria-labelledby="rw-password-title">
        ...
        <div class="rw-password-grid">
          ...
          <label><span>${t("newPassword")}</span><input type="password" autocomplete="new-password" minlength="6" data-rw-new-password></label>
          <label><span>${t("confirmPassword")}</span><input type="password" autocomplete="new-password" minlength="6" data-rw-confirm-password></label>
        </div>
  ...
  function bind(modal) {
    ...
    profileForm?.querySelector("[data-rw-change-password]")?.addEventListener("click",async(event)=>{
      ...
      setResult("");
      if(next.length<6)return setResult(t("passwordTooShort"),"error");
