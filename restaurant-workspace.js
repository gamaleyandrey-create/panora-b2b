...
(() => {
  ...
  function homeHtml() {
    ...
    return `<section class="rw-home">
      ...
      ${active ? (()=>{const priceSource=active.prices||account.prices||{};const note=notes.find(n=>n.orderId===active.id);return `<article class="rw-current-order"><div class="rw-current-order-main"><span>${t("activeOrder")}</span><div class="rw-current-order-title"><strong>${esc(orderNumber(active))}</strong><em>${esc(status(active))}</em></div><small><b>${t("delivery")}:</b> ${esc(localDate(active.deliveryDate || active.date))}</small>${active.bakeDate?`<small><b>${t("bake")}:</b> ${esc(localDate(active.bakeDate))}</small>`:""}<ul class="rw-current-order-items">${(active.items||[]).map(item=>{const qty=Number(item.quantity||0),unit=Number(priceSource[item.product]||0);return `<li><span>${esc(itemName(item.product))}</span><strong>${qty} ${t("pieces")} × ${portalMoney(unit)} = ${portalMoney(qty*unit)}</strong></li>`}).join("")}</ul>${note?`<small><b>${t("notes")}:</b> ${esc(noteNumber(note))}</small>`:""}</div><div class="rw-current-order-total"><span>${lang==="ru"?"Итого":lang==="es"?"Total":"Total"}</span><b>${portalMoney(orderTotal(active))}</b></div><div class="rw-current-order-actions"><button class="button button-ghost" data-rw-open-order="${esc(active.id)}">${lang==="ru"?"Открыть заказ":lang==="es"?"Abrir pedido":"Open order"}</button><button type="button" class="button button-ghost" data-order-messages="${esc(active.id)}" data-order-label="${esc(orderNumber(active))}">✉ ${lang==="ru"?"Сообщения":lang==="es"?"Mensajes":"Messages"}</button></div></article>`})() : ""}
  ...
  function ordersHtml() {
    ...
    return `<section class="rw-orders-page">
      ...
      ${(order.items||[]).length?`<ul>${order.items.map((item) => `<li><span>${esc(itemName(item.product))}</span><strong>${item.quantity} ${t("pieces")}<small>× ${portalMoney(Number((order.prices || account.prices)[item.product] || 0))}</small></strong></li>`).join("")}</ul>`:`<div class="rw-order-items-error" role="alert"><strong>${lang==="ru"?"Состав заказа не загружен":lang==="es"?"No se cargó la composición del pedido":"Order items were not loaded"}</strong><span>${lang==="ru"?"Обновите данные. Если состав не появится, заказ требует проверки пекарней.":lang==="es"?"Actualice los datos. Si los productos no aparecen, la panadería debe revisar el pedido.":"Refresh the data. If the items do not appear, the bakery must review the order."}</span></div>`}
      <footer><span>${t("bake")}: <strong>${esc(localDate(order.date))}</strong></span><div class="rw-order-footer-actions"><button type="button" class="rw-order-message-button" data-order-messages="${esc(order.id)}" data-order-label="${esc(orderNumber(order))}">✉ ${lang==="ru"?"Сообщения":lang==="es"?"Mensajes":"Messages"}</button>${canRestaurantCancel(order) ? `<button class="rw-cancel" data-rw-cancel="${esc(order.id)}">${lang === "ru" ? "Отменить заказ" : lang === "es" ? "Cancelar pedido" : "Cancel order"}</button>` : ""}</div></footer>
    </article>`;
