...
(() => {
  ...
  function profileHtml() {
    return `<section class="rw-profile-page"><aside class="rw-profile rw-profile-summary" aria-label="${esc(account.name)}">
      ...
      <section class="rw-settings-panel">
        ...
        <label class="rw-setting-language"><span>${t("language")}</span><select name="language"><option value="ru"${account.language === "ru" ? " selected" : ""}>Русский</option><option value="es"${account.language === "es" ? " selected" : ""}>Español</option><option value="en"${account.language === "en" ? " selected" : ""}>English</option></select><small>${lang==="ru"?"Этот язык используется в кабинете, уведомлениях и обычных документах партнёра. Бухгалтерские документы пекарни — на испанском.":lang==="es"?"Este idioma se usa en el área, avisos y documentos habituales del socio. Los documentos contables de la panadería están en español.":"This language is used for the account, notifications and standard partner documents. Bakery accounting documents are in Spanish."}</small></label>
  ...
  function bind(modal) {
    ...
    const productDocsPrint=modal.querySelector('[data-rw-print-product-docs]');
    if(productDocsPrint)productDocsPrint.onclick=()=>{const box=modal.querySelector('.rw-product-documents');if(!box)return;document.body.classList.add('rw-product-docs-printing');box.classList.add('rw-product-docs-print-target');const cleanup=()=>{document.body.classList.remove('rw-product-docs-printing');box.classList.remove('rw-product-docs-print-target');window.removeEventListener('afterprint',cleanup)};window.addEventListener('afterprint',cleanup);window.print();setTimeout(cleanup,1500)};
...
/* Panora 10.62 — Partner global header refresh / language / Push / logout. */
(()=>{
 ...
 refresh.addEventListener('click',async()=>{if(refresh.disabled)return;const c=copy(),label=refresh.querySelector('b');refresh.disabled=true;refresh.dataset.loading='1';delete refresh.dataset.success;if(label)label.textContent=c.refreshing;try{const results=await Promise.allSettled([Promise.resolve(window.panoraPortalCloud?.load?.()),Promise.resolve(window.panoraPublicCatalog?.refreshIfChanged?.()),Promise.resolve(window.panoraOrderMessages?.refreshUnread?.())]);const failed=results.find(result=>result.status==='rejected');if(failed)throw failed.reason||new Error('partner_refresh_failed');delete refresh.dataset.loading;refresh.dataset.success='1';if(label)label.textContent=c.updated;window.dispatchEvent(new CustomEvent('panora:restaurant-sync',{detail:{type:'synced',text:language()==='es'?'Actualizado':language()==='en'?'Updated':'Актуально'}}));window.dispatchEvent(new CustomEvent('panora:partner-global-refreshed'));setTimeout(()=>{delete refresh.dataset.success;if(label)label.textContent=copy().refresh},1100)}catch(error){console.warn('Panora partner global refresh',error);delete refresh.dataset.loading;delete refresh.dataset.success;if(label)label.textContent=copy().refresh}finally{refresh.disabled=false}});
