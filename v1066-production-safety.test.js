...
const c=context(seed,4),api=c.window.panoraProductionSafety;assert.equal(api.version,'10.66');assert.equal(api.build,10660);api.reconcile();const st=api.read();
...
console.log('v1066 production safety: 23 assertions passed');
...
// Release integration guards added for final 10.66 packaging.
{
  ...
  const assert2=(cond,msg)=>{ if(!cond) throw new Error('v1066 release guard: '+msg); };
  assert2(/ПРОИЗВОДСТВО И БЕЗОПАСНОСТЬ/i.test(admin),'admin production/safety group');
  assert2(/production-safety\.js\?v=10660/.test(admin),'admin module version');
  assert2(/production-safety\.js\?v=10660/.test(bakery),'bakery module version');
  ...
  console.log('v1066 final integration guards: 14 assertions passed');
}
