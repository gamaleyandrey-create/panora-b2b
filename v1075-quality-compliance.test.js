...
assert.deepEqual(build,{version:'10.78',build:10780,cache:10780});
...
for(const token of ["const VERSION='10.78',BUILD=10780",'partner_quality_cases','renderQuality()','renderLabs()','renderShelf()','renderPackaging()','renderInspection()','createCapaFromCase','Печать / PDF'])assert.ok(qc.includes(token),token);
const admin=fs.readFileSync('admin.html','utf8'),bakery=fs.readFileSync('bakery/index.html','utf8');
for(const html of [admin,bakery])for(const token of ['data-view="ps-quality"','data-view="ps-labs"','data-view="ps-shelf-life"','data-view="ps-packaging"','data-view="ps-inspection"','quality-compliance-v1075.css?v=10780','quality-compliance-v1075.js?v=10780'])assert.ok(html.includes(token),token);
...
for(const token of ['Panora 10.75 — compact collapsible Bakery navigation groups','admin-nav-submenu-long','grid-column:1/-1','max-height:48dvh'])assert.ok(adminCss.includes(token),token);const adminTheme=fs.readFileSync('admin-partner-theme.css','utf8');for(const token of ['Panora 10.77 — larger persistent collapsible Bakery navigation sections','admin-nav-major-toggle.has-active','Panora 10.78 — compact persistent Bakery navigation groups','min-height:42px!important','font-size:12.8px!important','min-height:44px!important','font-size:13px!important'])assert.ok(adminTheme.includes(token),token);
{
...
console.log('Panora 10.78 quality/compliance + navigation tests: OK');
