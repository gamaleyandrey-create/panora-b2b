...
assert.deepEqual(build,{version:'10.77',build:10770,cache:10770});
...
for(const token of ["const VERSION='10.77',BUILD=10770",'partner_quality_cases','renderQuality()','renderLabs()','renderShelf()','renderPackaging()','renderInspection()','createCapaFromCase','Печать / PDF'])assert.ok(qc.includes(token),token);
const admin=fs.readFileSync('admin.html','utf8'),bakery=fs.readFileSync('bakery/index.html','utf8');
for(const html of [admin,bakery])for(const token of ['data-view="ps-quality"','data-view="ps-labs"','data-view="ps-shelf-life"','data-view="ps-packaging"','data-view="ps-inspection"','quality-compliance-v1075.css?v=10770','quality-compliance-v1075.js?v=10770'])assert.ok(html.includes(token),token);
...
for(const token of ['Panora 10.75 — compact collapsible Bakery navigation groups','admin-nav-submenu-long','grid-column:1/-1','max-height:48dvh'])assert.ok(adminCss.includes(token),token);const adminTheme=fs.readFileSync('admin-partner-theme.css','utf8');for(const token of ['Panora 10.77 — larger persistent collapsible Bakery navigation sections','font-size:13.5px!important','font-size:14px!important','admin-nav-major-toggle.has-active'])assert.ok(adminTheme.includes(token),token);
{
...
console.log('Panora 10.77 quality/compliance + navigation tests: OK');
