...
assert.deepEqual(build,{version:'11.12',build:11120,cache:11120});
...
const productAdmin1112=fs.readFileSync('product-admin.js','utf8'),productAdminCss1112=fs.readFileSync('product-admin.css','utf8');
for(const html of [admin,bakery])for(const token of ['data-view="price-labels"','id="view-price-labels"','id="priceLabelBuilder"','product-admin.css?v=11120','product-admin.js?v=11120','горизонтальные и вертикальные этикетки'])assert.ok(html.includes(token),`11.12 label UI: ${token}`);
for(const token of ['Panora 11.12 — rich-text label editor, manual text blocks and visual layout controls.','const BUILD=11120','const PANORA_SITE=\'https://panora.es/\'','function eanCheck','function normalizeEan','function eanSvg','shape-rendering="crispEdges"','function thermalInk','color:{dark:codeInk(),light:\'#ffffff\'}','58x160','data-value="vertical"','Вертикальная','Preenvasado','Envasado para venta inmediata','ES · основной','Белый + чёрный','Белый + основной цвет','function validationIssues','allergensVerified','quidEs','function highlightedComposition','pl-allergen-inline','Información nutricional por 100 g','Pide en','PANORA.ES','function verticalSiteUrl','function renderedFitIssues','function measureLabelFit','Печать остановлена','function printLabels','function recordHistory','Редактировать макет','Свой размер','Обычный принтер','function captureLayoutFromPreview','function wireLayoutEditorPreview','function blockAttrs','function layoutSafetyIssues','plBlockRotate','plAddBrandBlock','plAddTaglineBlock','data-pl-align',"function addManualTextBlock(kind='text')",'Название блока','function dynamicFieldMap','function undoLayout','function redoLayout','function saveCurrentTemplate','На передний план','Заблокировать','function confirmPrintWarnings','Игнорировать предупреждения и печатать','blocking:[...new Set(blocking)]'])assert.ok(productAdmin1112.includes(token),`11.12 label logic: ${token}`);
for(const token of ['Panora 11.12 — rich text, own blocks, alignment/layers/templates and confirm-before-print','Panora 11.12 — thermal/standard printer preview and confirmable completeness warnings.','.pl-layout','.pl-label','.pl-template-full','.product-label-nutrition-grid','.pl-template-vertical','.pl-size-58x160','.pl-vertical-order','.pl-allergen-inline','.pl-internal-code','.pl-size-58x40 .pl-price small{font-size:2mm','.pl-custom-layout','.pl-resize-handle','.pl-printer-standard-color','.pl-grid-4','.pl-rich-toolbar','.pl-align-buttons','.pl-layout-adders','.pl-template-tools','.pl-text-editing','.pl-print-warning-dialog','.pl-confirm-title'])assert.ok(productAdminCss1112.includes(token),`11.12 label CSS: ${token}`);
const release1112=fs.readFileSync('RELEASE_PANORA_11_12.txt','utf8');
for(const token of ['Panora 11.12 FULL','GTIN/EAN-13','internal Panora cash-register code','58×160','Spanish remains the default','white + black','No BIO marking','No SQL migration required.'])assert.ok(release1112.includes(token),token);
assert.ok(!productAdmin1112.includes('BIO'), 'Product label code must not print BIO marking');

{
 const start=productAdmin1112.indexOf('function eanCheck'),end=productAdmin1112.indexOf('function latestLot',start);assert.ok(start>=0&&end>start,'EAN source slice');
 const context={String,Number};vm.runInNewContext(productAdmin1112.slice(start,end)+`;this.eanApi={eanCheck,normalizeEan,eanSvg};`,context);
...
{
 const start=productAdmin1112.indexOf('function defaultInternalCode'),end=productAdmin1112.indexOf('function internalProductCode',start);assert.ok(start>=0&&end>start,'internal-code source slice');
 const context={String,Math};vm.runInNewContext(productAdmin1112.slice(start,end)+`;this.code=defaultInternalCode;`,context);
...
{
 const start=productAdmin1112.indexOf('function verticalSiteUrl'),end=productAdmin1112.indexOf('function thermalInk',start);assert.ok(start>=0&&end>start,'vertical URL source slice');
 const context={PANORA_SITE:'https://panora.es/'};vm.runInNewContext(productAdmin1112.slice(start,end)+`;this.site=verticalSiteUrl();`,context);assert.equal(context.site,'https://panora.es/');
}
{
 const start=productAdmin1112.indexOf('function nutritionComplete'),end=productAdmin1112.indexOf('function needsQuid',start);assert.ok(start>=0&&end>start,'nutrition completeness source slice');
 const context={num:v=>Number(String(v??'').replace(',','.'))||0,String,Number};vm.runInNewContext(productAdmin1112.slice(start,end)+`;this.ok=nutritionComplete;`,context);
...
{
 const start=productAdmin1112.indexOf('function renderedFitIssues'),end=productAdmin1112.indexOf('async function measureLabelFit',start);assert.ok(start>=0&&end>start,'label fit source slice');
 const toggles=[];const context={builder:{size:'58x160'},builderDimensions:()=>({w:58,h:160}),currentLayout:()=>null,richTextEditable:()=>true};vm.runInNewContext(productAdmin1112.slice(start,end)+`;this.fit=renderedFitIssues;`,context);
...
{
 const start=productAdmin1112.indexOf('function validationIssues'),end=productAdmin1112.indexOf('function warnings',start);assert.ok(start>=0&&end>start,'validation source slice');
 const source=productAdmin1112.slice(start,end)+`;this.check=validationIssues;`;
 const common={
...
assert.ok(productAdmin1112.includes('${esc(c.internal)}: <b>${esc(internal)}</b>'),'package/full/vertical meta must print internal code');
assert.ok(productAdmin1112.includes('pl-internal-code'),'shelf price tag must print internal cash-register code');
for(const token of ['Ценник','Упаковка','Полная','Вертикальная'])assert.ok(productAdmin1112.includes(token),`template retained: ${token}`);
for(const token of ['58 × 40 мм · ценник','70 × 50 мм · компактно','100 × 50 мм','100 × 70 мм · рекомендовано','58 × 160 мм · вертикальная'])assert.ok(productAdmin1112.includes(token),`size retained/added: ${token}`);
assert.ok(productAdmin1112.includes("language:migrated?'es'"),'Spanish must be the migrated/default label language');
assert.ok(productAdmin1112.includes('if(confirmable.length&&!await confirmPrintWarnings(confirmable))return'),'missing label data must require explicit confirmation before print');

console.log('Panora 11.12 rich label editor + confirmable print warnings + compliance guards: OK');
