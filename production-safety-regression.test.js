...
const c=context(seed,4),api=c.window.panoraProductionSafety;assert.equal(api.version,'11.01');assert.equal(api.build,11010);api.reconcile();const st=api.read();
...
// Release integration guards added for final 10.66 packaging.
{
  ...
  assert2(/production-safety\.js\?v=11010/.test(admin),'admin module version');
  assert2(/production-safety\.js\?v=11010/.test(bakery),'bakery module version');
