...
const c=context(seed,4),api=c.window.panoraProductionSafety;assert.equal(api.version,'10.82');assert.equal(api.build,10820);api.reconcile();const st=api.read();
...
// Release integration guards added for final 10.66 packaging.
{
  ...
  assert2(/production-safety\.js\?v=10820/.test(admin),'admin module version');
  assert2(/production-safety\.js\?v=10820/.test(bakery),'bakery module version');
