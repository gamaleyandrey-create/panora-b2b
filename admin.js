...
async function openRetailOrderMessages(orderId){const order=readRetailOrders().find(item=>String(item.id)===String(orderId)),dialog=$('#retailOrderMessagesDialog'),title=$('#retailOrderMessagesTitle'),meta=$('#retailOrderMessagesMeta'),list=$('#retailOrderMessagesList'),fallback=$('#retailOrderFallbackActions');if(!dialog||!order)return;clearInterval(retailMessagePoll);retailMessageOrderId=String(orderId);title.textContent=`Сообщения · ${retailOrderNumber(order)}`;meta.textContent=[order.customerName,order.customerPhone,order.customerEmail].filter(Boolean).join(' · ');list.innerHTML='<div class="retail-event-loading">Загружаем сообщения…</div>';if(fallback)fallback.innerHTML=retailFallbackLinks(order);if(!dialog.open)dialog.showModal();await refreshRetailOrderMessages(orderId);retailMessagePoll=setInterval(()=>{if(dialog.open&&!document.hidden&&retailMessageOrderId)refreshRetailOrderMessages(retailMessageOrderId,{quiet:true})},5000)}
