...
/* Panora 6.24 — real retail orders + temporary reservations. One and only location: bakery. */
...
const RETAIL_DEFAULT_SETTINGS={
 ...
 contactPhone:'',contactEmail:'',pickupNote:''
};
...
function normalizeRetailSettings(saved={}){
 ...
 return {...RETAIL_DEFAULT_SETTINGS,...saved,enabled:saved.enabled!==false,location:'bakery',locationName:'Пекарня',
  ...
  contactPhone:String(saved.contactPhone||'').trim().slice(0,80),contactEmail:String(saved.contactEmail||'').trim().slice(0,160),pickupNote:String(saved.pickupNote||'').trim().slice(0,500)};
}
...
function retailStatusLabel(status){return({new:'Новый',confirmed:'Подтверждён',preparing:'Готовится',ready:'Готов',completed:'Выдан',cancelled:'Отменён'}[status]||status||'—')}
function retailPaymentLabel(order){const value=String(order?.paymentStatus||'pending');return({pending:'При получении',paid:'Оплачен',refunded:'Возврат',failed:'Ошибка'}[value]||value)}
function retailFulfillmentLabel(){return 'Самовывоз'}
...
function retailCloudRow(settings,{legacy=false}={}){
 ...
 if(!legacy)Object.assign(row,{pay_on_pickup:!!settings.payOnPickup,preorder_horizon_days:settings.preorderHorizonDays,pickup_lead_minutes:settings.pickupLeadMinutes,max_orders_per_slot:settings.maxOrdersPerSlot,
  contact_phone:settings.contactPhone||null,contact_email:settings.contactEmail||null,pickup_note:settings.pickupNote||null});
 return row;
}
function retailSettingsFromCloud(row){if(!row)return null;return normalizeRetailSettings({enabled:row.enabled,stockSales:row.stock_sales,preorders:row.preorders,pickup:row.pickup,delivery:row.delivery,onlinePayment:row.online_payment,payOnPickup:row.pay_on_pickup,
 ...
 pickupSlots:Array.isArray(row.pickup_slots)?row.pickup_slots:RETAIL_DEFAULT_SETTINGS.pickupSlots,contactPhone:row.contact_phone,contactEmail:row.contact_email,pickupNote:row.pickup_note})}
...
async function loadRetailSettingsCloud(){
 let rows=null,extended=true;try{rows=await retailAdminApi('retail_settings?id=eq.1&select=id,enabled,stock_sales,preorders,pickup,delivery,online_payment,pay_on_pickup,reservation_minutes,preorder_cutoff_hours,preorder_horizon_days,pickup_lead_minutes,max_orders_per_slot,pickup_slots,contact_phone,contact_email,pickup_note,updated_at&limit=1')}
 catch(error){extended=false;try{rows=await retailAdminApi('retail_settings?id=eq.1&select=id,enabled,stock_sales,preorders,pickup,delivery,online_payment,reservation_minutes,preorder_cutoff_hours,pickup_slots,updated_at&limit=1')}catch(second){const saved=$('#retailSettingsSaved');if(saved)saved.textContent='Локальные настройки · выполните SQL 6.24';return false}}
 if(rows?.[0])saveRetailSettingsLocal(retailSettingsFromCloud(rows[0]));renderRetailFoundation();const saved=$('#retailSettingsSaved');if(saved&&rows?.[0])saved.textContent=extended?'Настройки загружены из облака':'Выполните SQL 6.24 для рабочих розничных заказов';return !!rows?.[0];
}
...
async function loadRetailProductSettingsCloud(){
 ...
 catch(error){const status=$('#retailCatalogSaved');if(status)status.textContent='Локальный ассортимент · выполните SQL 6.24';return false}
}
...
function retailOrderFromCloud(row){return{id:String(row.id||''),number:Number(row.order_number||0),publicToken:String(row.public_token||''),source:String(row.source||'stock'),fulfillment:String(row.fulfillment||'pickup'),bakeDate:row.bake_date||'',pickupDate:row.pickup_date||'',slot:row.pickup_slot||'',customerName:row.customer_name||'',customerPhone:row.customer_phone||'',customerEmail:row.customer_email||'',comment:row.comment||'',status:String(row.status||'new'),paymentStatus:String(row.payment_status||'pending'),total:Number(row.total||0),createdAt:row.created_at||'',updatedAt:row.updated_at||'',completedAt:row.completed_at||'',cancelledAt:row.cancelled_at||'',items:(row.retail_order_items||[]).map(item=>({product:String(item.product_id||''),quantity:Math.max(0,Number(item.quantity||0)),unitPrice:Number(item.unit_price||0)}))}}
async function loadRetailOrdersCloud(){
 const status=$('#retailOrdersCloudStatus');
 try{const rows=await retailAdminApi('retail_orders?select=id,order_number,public_token,source,fulfillment,bake_date,pickup_date,pickup_slot,customer_name,customer_phone,customer_email,comment,status,payment_status,total,created_at,updated_at,completed_at,cancelled_at,retail_order_items(product_id,quantity,unit_price)&order=created_at.desc');saveRetailOrdersLocal((rows||[]).map(retailOrderFromCloud));if(status)status.textContent='Облако ✓';renderRetailOrderQueue();renderStock();return true}
 catch(error){if(status)status.textContent='Выполните SQL 6.24';return false}
}
async function updateRetailOrderStatusCloud(id,nextStatus){
 const payload={status:nextStatus,updated_at:new Date().toISOString()};await retailAdminApi(`retail_orders?id=eq.${encodeURIComponent(id)}`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify(payload)});await loadRetailOrdersCloud();window.dispatchEvent(new CustomEvent('panora:retail-orders-updated'))
}
...
function syncRetailSettingsForm(settings){const form=$('#retailSettingsForm');if(!form)return;['enabled','stockSales','preorders','pickup','delivery','onlinePayment','payOnPickup'].forEach(name=>{if(form.elements[name])form.elements[name].checked=!!settings[name]});['reservationMinutes','preorderCutoffHours','preorderHorizonDays','pickupLeadMinutes','maxOrdersPerSlot','contactPhone','contactEmail','pickupNote'].forEach(name=>{if(form.elements[name])form.elements[name].value=String(settings[name]??'')});if(form.elements.pickupSlots)form.elements.pickupSlots.value=(settings.pickupSlots||[]).join('\n');const label=$('#retailEnabledLabel');if(label)label.textContent=settings.enabled?'Включена':'Выключена'}
...
function retailNextAction(order){const status=retailOrderStatus(order);return({new:{status:'confirmed',label:'Подтвердить'},confirmed:{status:'preparing',label:'Готовить'},preparing:{status:'ready',label:'Готов'},ready:{status:'completed',label:'Выдать'}}[status]||null)}
function retailOrderPickupLabel(order){const date=String(order?.pickupDate||''),day=date?fmt(date,{weekday:'short',day:'numeric',month:'short'}):'—';return `${day}${order?.slot?` · ${order.slot}`:''}`}
...
function retailOrderRow(order){const action=retailNextAction(order),cancel=!['completed','cancelled'].includes(retailOrderStatus(order));return `<tr><td><strong>${adminEscape(retailOrderNumber(order))}</strong><small class="retail-order-sub">${adminEscape(retailSourceLabel(order))}</small></td><td>${adminEscape(retailOrderPickupLabel(order))}</td><td><strong>${adminEscape(order.customerName||'—')}</strong><small class="retail-order-sub">${adminEscape(order.customerPhone||'')}</small></td><td>${adminEscape(retailItemsLabel(order))}</td><td>${Number(order.total||0).toLocaleString('ru-RU',{minimumFractionDigits:2,maximumFractionDigits:2})} €</td><td>${adminEscape(retailPaymentLabel(order))}</td><td><span class="retail-order-status retail-status-${adminEscape(retailOrderStatus(order))}">${adminEscape(retailStatusLabel(retailOrderStatus(order)))}</span></td><td><div class="retail-order-actions">${action?`<button type="button" class="primary" data-retail-order-next="${adminEscape(order.id)}" data-next-status="${action.status}">${action.label}</button>`:''}${cancel?`<button type="button" class="secondary" data-retail-order-cancel="${adminEscape(order.id)}">Отменить</button>`:''}</div></td></tr>`}
function retailOrderCard(order){const action=retailNextAction(order),cancel=!['completed','cancelled'].includes(retailOrderStatus(order));return `<article class="retail-mobile-order-card"><div class="retail-mobile-order-head"><div><strong>${adminEscape(retailOrderNumber(order))}</strong><span>${adminEscape(retailSourceLabel(order))}</span></div><span class="retail-order-status retail-status-${adminEscape(retailOrderStatus(order))}">${adminEscape(retailStatusLabel(retailOrderStatus(order)))}</span></div><div class="retail-mobile-order-time">${adminEscape(retailOrderPickupLabel(order))}</div><div class="retail-mobile-order-customer"><strong>${adminEscape(order.customerName||'—')}</strong><span>${adminEscape([order.customerPhone,order.customerEmail].filter(Boolean).join(' · '))}</span></div><p>${adminEscape(retailItemsLabel(order))}</p><div class="retail-mobile-order-total"><strong>${Number(order.total||0).toLocaleString('ru-RU',{minimumFractionDigits:2,maximumFractionDigits:2})} €</strong><span>${adminEscape(retailPaymentLabel(order))}</span></div><div class="retail-mobile-order-actions">${action?`<button type="button" class="primary" data-retail-order-next="${adminEscape(order.id)}" data-next-status="${action.status}">${action.label}</button>`:''}${cancel?`<button type="button" class="secondary" data-retail-order-cancel="${adminEscape(order.id)}">Отменить</button>`:''}</div></article>`}
function bindRetailOrderActions(root){if(!root)return;root.querySelectorAll('[data-retail-order-next]').forEach(button=>button.onclick=async()=>{button.disabled=true;try{await updateRetailOrderStatusCloud(button.dataset.retailOrderNext,button.dataset.nextStatus)}catch(error){alert(`Не удалось изменить заказ: ${error.message||error}`)}finally{button.disabled=false}});root.querySelectorAll('[data-retail-order-cancel]').forEach(button=>button.onclick=async()=>{const order=readRetailOrders().find(o=>String(o.id)===String(button.dataset.retailOrderCancel));if(!confirm(`Отменить ${retailOrderNumber(order)}? Резерв хлеба будет освобождён.`))return;button.disabled=true;try{await updateRetailOrderStatusCloud(button.dataset.retailOrderCancel,'cancelled')}catch(error){alert(`Не удалось отменить заказ: ${error.message||error}`)}finally{button.disabled=false}})}
...
function renderRetailOrderQueue(){
 ...
 const banner=$('#retailStateBanner');if(banner)banner.dataset.state=settings.enabled?'on':'off';set('#retailStateTitle',settings.enabled?'Розничная витрина и приём заказов включены':'Розничная витрина выключена');set('#retailStateText',settings.enabled?'Покупатели могут создавать реальные заказы. «Из наличия» резервирует свободный хлеб. Предзаказы к выпечке пока ведутся отдельной очередью — подключение к производственному плану будет следующим этапом.':'Экран покупателя закрыт. Настройки, ассортимент и история заказов сохраняются.');
...
function renderRetailFoundation(){const settings=readRetailSettings();renderRetailOrderQueue();syncRetailSettingsForm(settings);renderRetailCatalogSettings()}
