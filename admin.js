...
/* Panora 6.28 — retail delivery for one Bakery. Pickup and one simple delivery zone share the same order pipeline. */
...
async function loadRetailSettingsCloud(){
 ...
 catch(error){extended=false;try{rows=await retailAdminApi('retail_settings?id=eq.1&select=id,enabled,stock_sales,preorders,pickup,delivery,online_payment,reservation_minutes,preorder_cutoff_hours,pickup_slots,updated_at&limit=1')}catch(second){const saved=$('#retailSettingsSaved');if(saved)saved.textContent='Локальные настройки · выполните SQL 6.28';return false}}
 if(rows?.[0])saveRetailSettingsLocal(retailSettingsFromCloud(rows[0]));renderRetailFoundation();const saved=$('#retailSettingsSaved');if(saved&&rows?.[0])saved.textContent=extended?'Настройки загружены из облака':'Выполните SQL 6.28 для доставки';return !!rows?.[0];
}
...
async function loadRetailOrdersCloud(){
 ...
 try{const rows=await retailAdminApi('retail_orders?select=id,order_number,public_token,source,fulfillment,bake_date,pickup_date,pickup_slot,customer_name,customer_phone,customer_email,comment,delivery_address,delivery_note,delivery_fee,status,payment_status,payment_method,payment_provider,total,created_at,updated_at,completed_at,cancelled_at,retail_order_items(product_id,quantity,unit_price)&order=created_at.desc');saveRetailOrdersLocal((rows||[]).map(retailOrderFromCloud));if(status)status.textContent='Облако ✓';renderRetailOrderQueue();renderStock();renderPlan();window.panoraRawStock?.render?.();if(typeof renderPurchase==='function')renderPurchase();return true}
