...
async function retailEnsureAdminPush({prompt=false}={}){const state=$('#retailAdminPushState'),settings=readRetailSettings();if(!('Notification'in window)||!('serviceWorker'in navigator)||!('PushManager'in window)){localStorage.removeItem('panora-admin-webpush-registered');if(state)state.textContent='Этот браузер не поддерживает Web Push';return false}if(!settings.pushAdminEnabled){localStorage.removeItem('panora-admin-webpush-registered');if(state)state.textContent='Push для пекарни выключен в настройках';return false}if(!settings.pushVapidPublicKey){localStorage.removeItem('panora-admin-webpush-registered');if(state)state.textContent='Добавьте VAPID public key в настройках';return false}try{let permission=Notification.permission;if(permission!=='granted'&&prompt)permission=await Notification.requestPermission();if(permission!=='granted'){localStorage.removeItem('panora-admin-webpush-registered');if(state)state.textContent='Разрешение на уведомления не предоставлено';return false}if(!window.panoraSupabaseSession?.access_token)return false;const reg=await navigator.serviceWorker.ready;let sub=await reg.pushManager.getSubscription();if(!sub){if(!prompt)return false;sub=await reg.pushManager.subscribe({userVisibleOnly:true,applicationServerKey:retailPushB64ToBytes(settings.pushVapidPublicKey)})}const json=sub.toJSON();await retailAdminApi('retail_push_subscriptions?on_conflict=endpoint',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=minimal'},body:JSON.stringify({audience:'admin',endpoint:json.endpoint,p256dh:json.keys?.p256dh||'',auth_key:json.keys?.auth||'',user_agent:navigator.userAgent,active:true,updated_at:new Date().toISOString()})});localStorage.setItem('panora-admin-webpush-registered','1');window.dispatchEvent(new CustomEvent('panora:admin-webpush-state',{detail:{active:true}}));if(state)state.textContent='Push включён на этом устройстве ✓';return true}catch(error){localStorage.removeItem('panora-admin-webpush-registered');window.dispatchEvent(new CustomEvent('panora:admin-webpush-state',{detail:{active:false,error:String(error?.message||error)}}));if(state)state.textContent=`Push: ${error.message||'ошибка подключения'}`;if(prompt)throw error;return false}}
async function retailEnableAdminPush(){return retailEnsureAdminPush({prompt:true})}
window.panoraAdminWebPush={enable:retailEnableAdminPush,ensure:()=>retailEnsureAdminPush({prompt:false})};
