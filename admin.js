...
function renderPlan(){
 ...
 const visible=plans.filter(p=>p.bakeDate>=iso(weekStart)&&p.bakeDate<=iso(end)&&(history?p.bakeDate<today:p.bakeDate>=today)).sort((a,b)=>a.bakeDate.localeCompare(b.bakeDate)),planned=visible.reduce((sum,p)=>sum+Number(p.planned||0),0),partnerOrdered=visible.reduce((sum,p)=>sum+Number(p.ordered||0),0),retailOrdered=visible.reduce((sum,p)=>sum+retailPreorderQuantity(p.bakeDate,p.product),0),ordered=partnerOrdered+retailOrdered;$('#plannedPieces').textContent=`${planned} ${t('pcs')}`;$('#orderedPieces').textContent=`${ordered} ${t('pcs')}`;$('#freePieces').textContent=`${Math.max(0,planned-ordered)} ${t('pcs')}`;
 $('#planList').innerHTML=visible.length?visible.map(p=>{const partner=Math.max(0,Number(p.ordered||0)),retail=retailPreorderQuantity(p.bakeDate,p.product),real=partner+retail,percent=p.planned?Math.min(100,Math.round(real/p.planned*100)):0,completion=bakeCompletionFor(p.bakeDate),readonly=history||p.bakeDate<today;return `<article class="plan-card ${completion?'bake-is-completed':''} ${readonly?'plan-history-readonly':''}"><div class="date"><strong>${fmt(p.bakeDate,{weekday:'short',day:'numeric',month:'short'})}</strong><small>${t('delivery')}: ${fmt(p.deliveryDate)}</small></div><div class="product"><strong>${productName(p.product)}</strong><small>${t('cutoffShort')}: ${new Date(p.cutoff).toLocaleString()}</small></div><div><strong>${p.planned} ${t('pcs')}</strong><small class="plan-demand-sources">Партнёры ${partner} · Розница ${retail} · Всего ${real}</small><div class="progress"><i style="width:${percent}%"></i></div></div><div><input class="ordered-input" data-order="${p.id}" type="number" min="0" value="${partner}" aria-label="${t('ordered')}" ${readonly||completion?'disabled title="История — только просмотр"':''}></div><span class="status ${completion?'completed':readonly?'history':p.open?'':'closed'}">${completion?'Завершена':readonly?'История':p.open?t('open'):t('closed')}</span><button class="icon-delete" data-cancel-plan-date="${p.bakeDate}" title="${readonly?'История — только просмотр':'Отменить весь день выпечки'}" aria-label="${readonly?'История — только просмотр':'Отменить весь день выпечки'}" ${readonly||completion?'disabled':''}>×</button></article>`}).join(''):`<article class="plan-card"><div>${history?'Нет прошедших выпечек в этой неделе.':t('empty')}</div></article>`;
...
$('#planForm').onsubmit=e=>{e.preventDefault();const form=$('#planForm'),f=new FormData(form),amounts={plain:Number(f.get('plainPlanned')||0),pumpkin:Number(f.get('pumpkinPlanned')||0)},error=$('#planError'),bakeDate=String(f.get('bakeDate')||''),deliveryDate=String(f.get('deliveryDate')||''),cutoff=cutoffToIso(f.get('cutoff'));error.textContent='';if(!form.reportValidity())return;if(deliveryDate<bakeDate){error.textContent='Дата доставки не может быть раньше даты выпечки.';form.deliveryDate.focus();return}if(!cutoff){error.textContent='Проверьте дату и время окончания приёма заказов.';form.cutoff.focus();return}if(new Date(cutoff)>=new Date(`${bakeDate}T23:59:59`)){error.textContent='Приём заказов должен завершиться до окончания дня выпечки.';form.cutoff.focus();return}if(!amounts.plain&&!amounts.pumpkin){error.textContent='Укажите количество обычного или тыквенного хлеба.';form.plainPlanned.focus();return}Object.entries(amounts).forEach(([product,planned])=>{if(!planned)return;const existing=plans.find(p=>p.bakeDate===bakeDate&&p.product===product);if(existing)Object.assign(existing,{deliveryDate,planned,cutoff,open:f.get('open')==='on'});else plans.push({id:crypto.randomUUID(),bakeDate,deliveryDate,product,planned,ordered:0,cutoff,open:f.get('open')==='on'})});store('panora-production-plans',plans);$('#planDialog').close();renderAll()};
