...
function retailCloudRow(settings,{legacy=false}={}){
 ...
 if(!legacy)Object.assign(row,{pay_on_pickup:!!settings.payOnPickup,preorder_online_discount_enabled:!!settings.preorderOnlineDiscountEnabled,preorder_online_discount_percent:Number(settings.preorderOnlineDiscountPercent||0),preorder_online_discount_min_days:Number(settings.preorderOnlineDiscountMinDays||2),preorder_horizon_days:settings.preorderHorizonDays,pickup_lead_minutes:settings.pickupLeadMinutes,max_orders_per_slot:settings.maxOrdersPerSlot,
...
function retailSettingsFromCloud(row){if(!row)return null;return normalizeRetailSettings({enabled:row.enabled,stockSales:row.stock_sales,preorders:row.preorders,pickup:row.pickup,delivery:row.delivery,onlinePayment:row.online_payment,payOnPickup:row.pay_on_pickup,preorderOnlineDiscountEnabled:row.preorder_online_discount_enabled===true,preorderOnlineDiscountPercent:row.preorder_online_discount_percent,preorderOnlineDiscountMinDays:row.preorder_online_discount_min_days,
