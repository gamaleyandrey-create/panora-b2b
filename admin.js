/* Panora 9.89 CLEAN BASE IV — one-time local operational reset.
...
(()=>{
  const resetMarker='panora-clean-production-v9890';
  try{
...
Object.assign(TEXT.ru,{plan:'Календарь выпечки',planTitle:'Календарь выпечки',planText:'Назначайте дни выпечки, отмечайте виды хлеба и при необходимости указывайте рабочий план в штуках.',planned:'К выпечке',reserve:'Сверх заказов'});
Object.assign(TEXT.en,{plan:'Bake calendar',planTitle:'Bake calendar',planText:'Schedule bake days, select the breads to be baked and optionally set a working quantity plan.',planned:'To bake',reserve:'Extra over orders'});
Object.assign(TEXT.es,{plan:'Calendario de horneado',planTitle:'Calendario de horneado',planText:'Programa los días de horneado, marca los panes previstos y, si hace falta, indica un plan de unidades.',planned:'A hornear',reserve:'Extra sobre pedidos'});
...
function localDateTimeInputValue(date){const d=new Date(date),pad=v=>String(v).padStart(2,'0');return `${d.getFullYear()}-${pad(d.getMonth()+1)}-${pad(d.getDate())}T${pad(d.getHours())}:${pad(d.getMinutes())}`}
function cutoffToIso(value){const raw=String(value||'').trim();if(!raw)return'';const parsed=new Date(raw);return Number.isFinite(parsed.getTime())?parsed.toISOString():''}
...
const RETAIL_DEFAULT_SETTINGS={
 ...
 onlinePayment:true,payOnPickup:true,preorderOnlineDiscountEnabled:false,preorderOnlineDiscountPercent:5,preorderOnlineDiscountMinDays:2,reservationMinutes:15,preorderCutoffHours:24,preorderHorizonDays:7,
...
function normalizeRetailSettings(saved={}){
 ...
 return {...RETAIL_DEFAULT_SETTINGS,...saved,enabled:saved.enabled!==false,location:'bakery',locationName:'Пекарня',
  ...
  onlinePayment:saved.onlinePayment!==false,payOnPickup:saved.payOnPickup!==false,preorderOnlineDiscountEnabled:!!saved.preorderOnlineDiscountEnabled,preorderOnlineDiscountPercent:Math.min(30,Math.max(0,Number(saved.preorderOnlineDiscountPercent??5))),preorderOnlineDiscountMinDays:Math.min(14,Math.max(1,Number(saved.preorderOnlineDiscountMinDays??2))),
...
function retailReturnedQuantityForItem(order,item,index){
 const sold=Math.max(0,Number(item?.quantity||0));if(!sold)return 0;
 const returned=(Array.isArray(movements)?movements:[]).filter(m=>retailReturnMovementMatches(m,order,item,index)).reduce((sum,m)=>sum+Math.max(0,Math.abs(Number(m?.quantity||0))),0);
 return Math.min(sold,returned);
}
function retailOrderReturnProgress(order){
 const items=Array.isArray(order?.items)?order.items:[],sold=items.reduce((sum,item)=>sum+Math.max(0,Number(item?.quantity||0)),0),returned=items.reduce((sum,item,index)=>sum+retailReturnedQuantityForItem(order,item,index),0);
 return{sold,returned,remaining:Math.max(0,sold-returned),complete:sold>0&&returned>=sold-0.0001};
}
function retailOrderReturnedToStock(order){return retailOrderReturnProgress(order).complete}
function retailPaymentLabel(order){const value=String(order?.paymentStatus||'pending'),method=String(order?.paymentMethod||'pickup');if(value==='paid')return 'Оплачен';if(value==='refunded'){const progress=retailOrderReturnProgress(order);if(progress.complete)return 'Возврат · хлеб на складе';if(progress.returned>0)return `Возврат · на складе ${progress.returned}/${progress.sold} шт.`;return 'Возврат оплаты'}if(value==='failed')return 'Ошибка оплаты';return method==='online'?'Ожидает онлайн-оплаты':'При получении'}
...
function retailCloudRow(settings,{level='full'}={}){
 ...
 if(level!=='legacy')Object.assign(row,{pay_on_pickup:!!settings.payOnPickup,preorder_horizon_days:settings.preorderHorizonDays,pickup_lead_minutes:settings.pickupLeadMinutes,max_orders_per_slot:settings.maxOrdersPerSlot,
  contact_phone:settings.contactPhone||null,contact_email:settings.contactEmail||null,pickup_note:settings.pickupNote||null,payment_provider:settings.paymentProvider||'none',delivery_min_total:settings.deliveryMinTotal,delivery_fee:settings.deliveryFee,delivery_free_from:settings.deliveryFreeFrom,max_deliveries_per_slot:settings.maxDeliveriesPerSlot,delivery_slots:settings.deliverySlots});
 if(['633','635','full'].includes(level))Object.assign(row,{notify_order_received:!!settings.notifyOrderReceived,notify_ready:!!settings.notifyReady,notify_delivery:!!settings.notifyDelivery,notify_cancelled:!!settings.notifyCancelled});
 if(['635','full'].includes(level))Object.assign(row,{contact_whatsapp:settings.contactWhatsApp||null,contact_telegram:settings.contactTelegram||null,fallback_whatsapp:!!settings.fallbackWhatsApp,fallback_telegram:!!settings.fallbackTelegram,fallback_sms:!!settings.fallbackSms,fallback_email:!!settings.fallbackEmail,push_customer_enabled:!!settings.pushCustomerEnabled,push_admin_enabled:!!settings.pushAdminEnabled,push_new_order:!!settings.pushNewOrder,push_new_message:!!settings.pushNewMessage,push_quiet_from:settings.pushQuietFrom||null,push_quiet_to:settings.pushQuietTo||null,push_vapid_public_key:settings.pushVapidPublicKey||null,push_timezone:settings.pushTimeZone||Intl.DateTimeFormat().resolvedOptions().timeZone||'UTC'});
 if(level==='full')Object.assign(row,{preorder_online_discount_enabled:!!settings.preorderOnlineDiscountEnabled,preorder_online_discount_percent:Number(settings.preorderOnlineDiscountPercent||0),preorder_online_discount_min_days:Number(settings.preorderOnlineDiscountMinDays||2)});
 return row;
}
function retailSettingsFromCloud(row,{preservePush=false}={}){if(!row)return null;const local=readRetailSettings(),pick=(cloud,localValue)=>cloud===undefined?localValue:cloud;return normalizeRetailSettings({enabled:row.enabled,stockSales:row.stock_sales,preorders:row.preorders,pickup:row.pickup,delivery:row.delivery,onlinePayment:row.online_payment,payOnPickup:row.pay_on_pickup,preorderOnlineDiscountEnabled:row.preorder_online_discount_enabled===undefined?local.preorderOnlineDiscountEnabled:row.preorder_online_discount_enabled===true,preorderOnlineDiscountPercent:pick(row.preorder_online_discount_percent,local.preorderOnlineDiscountPercent),preorderOnlineDiscountMinDays:pick(row.preorder_online_discount_min_days,local.preorderOnlineDiscountMinDays),
 ...
 pickupSlots:Array.isArray(row.pickup_slots)?row.pickup_slots:RETAIL_DEFAULT_SETTINGS.pickupSlots,deliveryMinTotal:row.delivery_min_total,deliveryFee:row.delivery_fee,deliveryFreeFrom:row.delivery_free_from,maxDeliveriesPerSlot:row.max_deliveries_per_slot,deliverySlots:Array.isArray(row.delivery_slots)?row.delivery_slots:RETAIL_DEFAULT_SETTINGS.deliverySlots,contactPhone:row.contact_phone,contactEmail:row.contact_email,pickupNote:row.pickup_note,paymentProvider:row.payment_provider,notifyOrderReceived:pick(row.notify_order_received,local.notifyOrderReceived),notifyReady:pick(row.notify_ready,local.notifyReady),notifyDelivery:pick(row.notify_delivery,local.notifyDelivery),notifyCancelled:pick(row.notify_cancelled,local.notifyCancelled),contactWhatsApp:pick(row.contact_whatsapp,local.contactWhatsApp),contactTelegram:pick(row.contact_telegram,local.contactTelegram),fallbackWhatsApp:pick(row.fallback_whatsapp,local.fallbackWhatsApp),fallbackTelegram:pick(row.fallback_telegram,local.fallbackTelegram),fallbackSms:pick(row.fallback_sms,local.fallbackSms),fallbackEmail:pick(row.fallback_email,local.fallbackEmail),pushCustomerEnabled:preservePush?local.pushCustomerEnabled:pick(row.push_customer_enabled,local.pushCustomerEnabled),pushAdminEnabled:preservePush?local.pushAdminEnabled:pick(row.push_admin_enabled,local.pushAdminEnabled),pushNewOrder:preservePush?local.pushNewOrder:pick(row.push_new_order,local.pushNewOrder),pushNewMessage:preservePush?local.pushNewMessage:pick(row.push_new_message,local.pushNewMessage),pushQuietFrom:preservePush?local.pushQuietFrom:pick(row.push_quiet_from,local.pushQuietFrom),pushQuietTo:preservePush?local.pushQuietTo:pick(row.push_quiet_to,local.pushQuietTo),pushVapidPublicKey:preservePush?local.pushVapidPublicKey:pick(row.push_vapid_public_key,local.pushVapidPublicKey),pushTimeZone:preservePush?local.pushTimeZone:(pick(row.push_timezone,local.pushTimeZone)||Intl.DateTimeFormat().resolvedOptions().timeZone||'UTC')})}
...
async function loadRetailSettingsCloud(){
 ...
 try{rows=await retailAdminApi('retail_settings?id=eq.1&select=id,enabled,stock_sales,preorders,pickup,delivery,online_payment,pay_on_pickup,preorder_online_discount_enabled,preorder_online_discount_percent,preorder_online_discount_min_days,reservation_minutes,preorder_cutoff_hours,preorder_horizon_days,pickup_lead_minutes,max_orders_per_slot,pickup_slots,contact_phone,contact_email,contact_whatsapp,contact_telegram,fallback_whatsapp,fallback_telegram,fallback_sms,fallback_email,pickup_note,payment_provider,delivery_min_total,delivery_fee,delivery_free_from,max_deliveries_per_slot,delivery_slots,notify_order_received,notify_ready,notify_delivery,notify_cancelled,push_customer_enabled,push_admin_enabled,push_new_order,push_new_message,push_quiet_from,push_quiet_to,push_vapid_public_key,push_timezone,updated_at&limit=1');level='1031'}
 catch(error){
  level='635';
 ...
 if(rows?.[0])saveRetailSettingsLocal(retailSettingsFromCloud(rows[0],{preservePush:!['1031','635'].includes(level)}));renderRetailFoundation();const saved=$('#retailSettingsSaved');if(saved&&rows?.[0])saved.textContent=level==='1031'?'Настройки и цена предзаказа загружены из облака':level==='635'?'Настройки загружены · выполните SQL 10.32 для цены предзаказа':level==='633'?'Настройки загружены · Push оставлен с этого устройства · выполните SQL 6.35':level==='628'?'Настройки загружены · Push оставлен с этого устройства · выполните SQL 6.33–6.35':'Обновите SQL розницы · Push оставлен с этого устройства';return !!rows?.[0];
}
async function saveRetailSettingsCloud(settings){
 const levels=['full','635','633','628','legacy'];let lastError=null;
 for(const level of levels){
  try{const rows=await retailAdminApi('retail_settings?on_conflict=id',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=representation'},body:JSON.stringify(retailCloudRow(settings,{level}))});return {row:rows?.[0]||null,extended:level!=='legacy',level,pushSaved:['full','635'].includes(level)}}
  catch(error){lastError=error}
 }
 throw lastError||new Error('Не удалось сохранить настройки розницы');
}
...
async function updateRetailPaymentStatusCloud(id,nextStatus){
 ...
 await loadRetailOrdersCloud();const current=readRetailOrders().find(order=>String(order.id)===String(id));if(!current||String(current.paymentStatus||'')!==String(nextStatus))throw new Error('Статус оплаты не подтвердился после синхронизации');window.dispatchEvent(new CustomEvent('panora:retail-payments-updated'))
}
function retailReturnQuantitiesPrompt(order){
 const items=Array.isArray(order?.items)?order.items:[],quantities={};
 for(let index=0;index<items.length;index+=1){
  const item=items[index],sold=Math.max(0,Number(item?.quantity||0)),returned=retailReturnedQuantityForItem(order,item,index),remaining=Math.max(0,sold-returned);if(remaining<=0)continue;
  const raw=prompt(`Сколько вернуть на склад: ${retailItemName(item?.product)}?\nПродано ${sold} шт. · уже возвращено ${returned} шт. · осталось максимум ${remaining} шт.`,String(remaining));if(raw===null)return null;
  const qty=Math.max(0,Number(String(raw).replace(',','.'))||0);if(qty>remaining+0.0001){alert(`Можно вернуть не больше ${remaining} шт.`);return retailReturnQuantitiesPrompt(order)}if(qty>0)quantities[index]=qty;
 }
 return quantities;
}
async function returnRetailOrderToFinishedStock(order,quantitiesByIndex=null){
 if(window.panoraSupabaseSession?.access_token){const refreshed=await loadFinishedStockMovementsCloud();if(!refreshed)throw new Error('Не удалось обновить облачные движения склада перед возвратом. Повторите после синхронизации.')}
 const items=Array.isArray(order?.items)?order.items:[],now=new Date().toISOString(),date=stockLocalDate(),number=retailOrderNumber(order),requested=quantitiesByIndex&&typeof quantitiesByIndex==='object'?quantitiesByIndex:null;
 const rows=items.map((item,index)=>{const sold=Math.max(0,Number(item?.quantity||0)),already=retailReturnedQuantityForItem(order,item,index),remaining=Math.max(0,sold-already),asked=requested?Math.max(0,Number(requested[index]||0)):remaining,quantity=Math.min(remaining,asked);return{id:crypto.randomUUID(),marker:retailReturnMovementMarker(order,item,index),date,product:String(item?.product||''),type:'returned',quantity,note:`Возврат розничного заказа ${number} · ${retailReturnMovementMarker(order,item,index)}`,createdAt:now,retailOrderId:String(order?.id||''),item,index}}).filter(row=>row.product&&row.quantity>0);
 if(!rows.length)return true;
 if(window.panoraSupabaseSession?.access_token){
  const cloudRows=rows.map(row=>({id:row.id,movement_date:row.date,product_id:row.product,movement_type:'returned',quantity:row.quantity,note:row.note,created_at:row.createdAt,updated_at:now,updated_by:window.panoraSupabaseSession?.user?.id||null}));
  ...
  const confirmed=rows.every(row=>(Array.isArray(saved)?saved:[]).some(server=>String(server?.id||'')===row.id&&String(server?.movement_type||'')==='returned'&&String(server?.note||'').includes(row.marker)&&Math.abs(Number(server?.quantity||0))>=row.quantity));if(!confirmed)throw new Error('Сервер не подтвердил возврат хлеба на склад');
  movements.push(...rows.map(({item,index,marker,...row})=>row));setLocalStorageSafely('panora-stock-movements',JSON.stringify(movements));
 }else{
  movements.push(...rows.map(({item,index,marker,...row})=>row));setLocalStorageSafely('panora-stock-movements',JSON.stringify(movements));
 }
...
let finishedStockCloudLoading=null;
const finishedStockMovementSignature=list=>JSON.stringify((Array.isArray(list)?list:[]).map(item=>({
 id:String(item?.id||''),date:String(item?.date||''),product:String(item?.product||''),type:String(item?.type||''),
 quantity:Math.abs(Number(item?.quantity||0)),note:String(item?.note||''),createdAt:String(item?.createdAt||'')
...
let retailMessageOrderId='',retailMessageEventRefreshTimer=0;
...
async function refreshRetailOrderMessages(orderId,{quiet=false}={}){const list=$('#retailOrderMessagesList');if(!list||!orderId)return;try{const messages=await loadRetailOrderMessagesCloud(orderId);list.innerHTML=messages?.length?messages.map(m=>`<article class="retail-chat-message ${m.sender_role==='bakery'?'is-bakery':m.sender_role==='system'?'is-system':'is-customer'}"><span>${m.sender_role==='bakery'?'Пекарня':m.sender_role==='system'?'Panora':'Покупатель'}</span><p>${adminEscape(m.body)}</p><small>${adminEscape(retailMessageTime(m.created_at))}</small></article>`).join(''):'<div class="retail-event-loading">Сообщений пока нет. Напишите покупателю первым.</div>';list.scrollTop=list.scrollHeight;await markRetailOrderMessagesRead(orderId);retailUnreadMessages.delete(String(orderId));renderRetailOrderQueue()}catch(error){if(!quiet)list.innerHTML=`<div class="retail-event-loading">Сообщения недоступны · ${adminEscape(error.message||'выполните SQL 6.36')}</div>`}}
async function retailSyncOrderMessagePush(){
 const state=$('#retailOrderMessagesPushState'),button=$('#retailOrderMessagesPushToggle');
 if(!state||!button)return false;
 if(!window.panoraAdminWebPush?.status){state.textContent='Push недоступен';button.disabled=true;return false}
 button.disabled=true;state.textContent='Проверяем Push…';
 try{
  const info=await window.panoraAdminWebPush.status();
  const active=Boolean(info?.active);
  button.dataset.active=active?'1':'0';
  button.textContent=active?'Отключить Push':'Включить Push';
  state.textContent=active?'Push включён ✓':'Push выключен';
  return active;
 }catch(error){
  button.dataset.active='0';button.textContent='Включить Push';state.textContent=`Push: ${error.message||'ошибка'}`;
  return false;
 }finally{button.disabled=false}
}
async function retailToggleOrderMessagePush(){
 const button=$('#retailOrderMessagesPushToggle'),state=$('#retailOrderMessagesPushState');
 if(!button||!window.panoraAdminWebPush)return;
 button.disabled=true;if(state)state.textContent='Обновляем Push…';
 try{
  if(button.dataset.active==='1')await window.panoraAdminWebPush.disable();
  else await window.panoraAdminWebPush.enable();
 }catch(error){if(state)state.textContent=`Push: ${error.message||'ошибка'}`}
 await retailSyncOrderMessagePush();
}
async function openRetailOrderMessages(orderId){const order=readRetailOrders().find(item=>String(item.id)===String(orderId)),dialog=$('#retailOrderMessagesDialog'),title=$('#retailOrderMessagesTitle'),meta=$('#retailOrderMessagesMeta'),list=$('#retailOrderMessagesList'),fallback=$('#retailOrderFallbackActions');if(!dialog||!order)return;retailMessageOrderId=String(orderId);title.textContent=`Связь · ${retailOrderNumber(order)}`;meta.textContent=[order.customerName,order.customerPhone,order.customerEmail].filter(Boolean).join(' · ');list.innerHTML='<div class="retail-event-loading">Загружаем сообщения…</div>';if(fallback)fallback.innerHTML=retailFallbackLinks(order);if(!dialog.open)dialog.showModal();retailSyncOrderMessagePush().catch(()=>{});await refreshRetailOrderMessages(orderId)}
function retailOrderRow(order){const action=retailNextAction(order),cancel=!['completed','cancelled'].includes(retailOrderStatus(order));return `<tr><td><strong>${adminEscape(retailOrderNumber(order))}</strong><small class="retail-order-sub">${adminEscape(retailSourceLabel(order))}</small></td><td>${adminEscape(retailOrderPickupLabel(order))}${order.fulfillment==='delivery'&&order.deliveryAddress?`<small class="retail-order-sub">${adminEscape(order.deliveryAddress)}</small>`:''}</td><td><strong>${adminEscape(order.customerName||'—')}</strong><small class="retail-order-sub">${adminEscape(order.customerPhone||'')}</small></td><td>${adminEscape(retailItemsLabel(order))}</td><td>${Number(order.total||0).toLocaleString('ru-RU',{minimumFractionDigits:2,maximumFractionDigits:2})} €</td><td>${adminEscape(retailPaymentLabel(order))}</td><td><span class="retail-order-status retail-status-${adminEscape(retailOrderStatus(order))}">${adminEscape(retailStatusLabel(retailOrderStatus(order),order))}</span></td><td><div class="retail-order-actions">${action?`<button type="button" class="primary" data-retail-order-next="${adminEscape(order.id)}" data-next-status="${action.status}">${action.label}</button>`:''}${order.paymentStatus==='pending'?`<button type="button" class="secondary" data-retail-payment="${adminEscape(order.id)}" data-payment-status="paid">Оплачено</button>`:order.paymentStatus==='paid'?`<button type="button" class="secondary" data-retail-payment="${adminEscape(order.id)}" data-payment-status="refunded">Возврат</button>`:''}${order.paymentStatus==='refunded'&&retailOrderStatus(order)==='completed'&&!retailOrderReturnedToStock(order)?`<button type="button" class="secondary" data-retail-stock-return="${adminEscape(order.id)}">Хлеб на склад</button>`:''}<button type="button" class="secondary" data-retail-order-messages="${adminEscape(order.id)}">Связь${retailUnreadMessages.get(String(order.id))?` (${retailUnreadMessages.get(String(order.id))})`:``}</button><button type="button" class="secondary" data-retail-order-events="${adminEscape(order.id)}">История</button>${order.publicToken?`<button type="button" class="secondary" data-retail-order-link="${adminEscape(order.publicToken)}">Ссылка</button>`:''}${cancel?`<button type="button" class="secondary" data-retail-order-cancel="${adminEscape(order.id)}">Отменить</button>`:''}</div></td></tr>`}
...
function scheduleRetailMessageEventRefresh(orderId=''){
 clearTimeout(retailMessageEventRefreshTimer);retailMessageEventRefreshTimer=setTimeout(async()=>{
  if(document.hidden||!navigator.onLine)return;
  await loadRetailUnreadMessagesCloud().catch(()=>{});renderRetailOrderQueue();
  const dialog=$('#retailOrderMessagesDialog'),target=String(orderId||'');
  if(dialog?.open&&retailMessageOrderId&&(!target||target===String(retailMessageOrderId)))await refreshRetailOrderMessages(retailMessageOrderId,{quiet:true});
 },280)
}
window.addEventListener('panora:realtime-retail-message',event=>scheduleRetailMessageEventRefresh(event.detail?.new?.order_id||event.detail?.record?.order_id||''));
const retailPushLooksLikeMessage=data=>{const signal=`${data?.kind||data?.type||data?.event_type||data?.tag||''} ${data?.title||''} ${data?.body||''}`.toLowerCase();return /(message|chat|сообщ|mensaje)/i.test(signal)};
navigator.serviceWorker?.addEventListener?.('message',event=>{if(event.data?.type==='PANORA_PUSH_RECEIVED'&&retailPushLooksLikeMessage(event.data.payload||{}))scheduleRetailMessageEventRefresh(event.data?.payload?.order_id||event.data?.payload?.orderId||'')});
function bindRetailOrderActions(root){if(!root)return;root.querySelectorAll('[data-retail-order-next]').forEach(button=>button.onclick=async()=>{button.disabled=true;try{await updateRetailOrderStatusCloud(button.dataset.retailOrderNext,button.dataset.nextStatus)}catch(error){alert(`Не удалось изменить заказ: ${error.message||error}`)}finally{button.disabled=false}});root.querySelectorAll('[data-retail-payment]').forEach(button=>button.onclick=async()=>{const next=button.dataset.paymentStatus,order=readRetailOrders().find(o=>String(o.id)===String(button.dataset.retailPayment));let returnToStock=false;if(next==='refunded'){if(!order||!confirm(`Оформить возврат оплаты по ${retailOrderNumber(order)}?`))return;if(retailOrderStatus(order)==='completed')returnToStock=confirm('Хлеб физически возвращён в пекарню и пригоден к повторной продаже?\n\nОК — вернуть хлеб на склад и отменить его COGS.\nОтмена — вернуть только деньги; себестоимость останется фактической потерей.')}button.disabled=true;try{await updateRetailPaymentStatusCloud(button.dataset.retailPayment,next);if(next==='refunded'&&returnToStock){try{await returnRetailOrderToFinishedStock(order)}catch(stockError){alert(`Оплата возвращена, но хлеб НЕ добавлен на склад: ${stockError.message||stockError}. Используйте кнопку «Хлеб на склад» в этом заказе после проверки облака.`)}}}catch(error){alert(`Не удалось изменить оплату: ${error.message||error}`)}finally{button.disabled=false}});root.querySelectorAll('[data-retail-stock-return]').forEach(button=>button.onclick=async()=>{const order=readRetailOrders().find(o=>String(o.id)===String(button.dataset.retailStockReturn));if(!order)return;const quantities=retailReturnQuantitiesPrompt(order);if(quantities===null||!Object.keys(quantities).length)return;button.disabled=true;try{await returnRetailOrderToFinishedStock(order,quantities)}catch(error){alert(`Не удалось вернуть хлеб на склад: ${error.message||error}`)}finally{button.disabled=false}});root.querySelectorAll('[data-retail-order-messages]').forEach(button=>button.onclick=()=>openRetailOrderMessages(button.dataset.retailOrderMessages));root.querySelectorAll('[data-retail-order-events]').forEach(button=>button.onclick=()=>openRetailOrderEvents(button.dataset.retailOrderEvents));root.querySelectorAll('[data-retail-order-link]').forEach(button=>button.onclick=()=>{const url=new URL('retail-order.html',location.href);url.searchParams.set('t',button.dataset.retailOrderLink);window.open(url.toString(),'_blank','noopener')});root.querySelectorAll('[data-retail-order-cancel]').forEach(button=>button.onclick=async()=>{const order=readRetailOrders().find(o=>String(o.id)===String(button.dataset.retailOrderCancel));if(String(order?.paymentStatus||'')==='paid'){alert(`Сначала оформите возврат оплаты по ${retailOrderNumber(order)}, затем отмените заказ.`);return}if(!confirm(`Отменить ${retailOrderNumber(order)}? Резерв хлеба будет освобождён.`))return;button.disabled=true;try{await updateRetailOrderStatusCloud(button.dataset.retailOrderCancel,'cancelled')}catch(error){alert(`Не удалось отменить заказ: ${error.message||error}`)}finally{button.disabled=false}})}
...
function retailAnalyticsEventDate(order){
 ...
 // Panora 7.11: completed/cancelled timestamps come from cloud in UTC; analytics uses local bakery dates.
...
function bindAdminNavGroup(toggleId,menuId,key,{defaultOpen=false,legacyKey=''}={}){const toggle=$(toggleId),menu=$(menuId);if(!toggle||!menu)return;const state=readAdminNavGroupState(),hasStored=Object.prototype.hasOwnProperty.call(state,key),hasLegacy=!hasStored&&legacyKey&&Object.prototype.hasOwnProperty.call(state,legacyKey),hasActive=!!menu.querySelector('[data-view].active'),open=hasStored?!!state[key]:hasLegacy?!!state[legacyKey]:(hasActive||!!defaultOpen);setAdminNavGroup(toggle,menu,key,open,{persist:false});toggle.onclick=()=>setAdminNavGroup(toggle,menu,key,toggle.getAttribute('aria-expanded')!=='true');menu.querySelectorAll('[data-view]').forEach(button=>button.addEventListener('click',()=>setAdminNavGroup(toggle,menu,key,true)));}
...
function bindAdminNavGroups(){ADMIN_NAV_GROUPS.forEach(({toggle,menu,key,defaultOpen,legacyKey})=>bindAdminNavGroup(toggle,menu,key,{defaultOpen,legacyKey}));syncAdminNavGroupActive()}
...
function bindRetailFoundation(){
 ...
 renderRetailFoundation();const cloudLoad=()=>Promise.allSettled([loadRetailSettingsCloud(),loadRetailProductSettingsCloud(),syncAndLoadFinishedStockMovementsCloud(),loadRetailOrdersCloud()]);window.addEventListener('panora:authenticated',cloudLoad,{once:true});if(window.panoraSupabaseSession?.access_token)setTimeout(cloudLoad,0);['panora:products-changed','panora:public-products-changed','panora:retail-catalog-updated'].forEach(name=>window.addEventListener(name,renderRetailCatalogSettings));window.addEventListener('panora:stock-movements-changed',()=>syncAndLoadFinishedStockMovementsCloud());window.addEventListener('storage',event=>{if(['panora-products',RETAIL_PRODUCT_SETTINGS_KEY,RETAIL_SETTINGS_KEY,RETAIL_ORDERS_KEY].includes(event.key))renderRetailFoundation()});let retailWakeAt=0,retailWakePromise=null;const activeRetailView=()=>document.querySelector('.view.active')?.id||'';const retailViewActive=()=>/^view-retail-/.test(activeRetailView());const refreshVisibleBakeryCloud=()=>{if(!window.panoraSupabaseSession?.access_token||!retailViewActive())return Promise.resolve(false);const now=Date.now();if(retailWakePromise)return retailWakePromise;if(now-retailWakeAt<15000)return Promise.resolve(false);retailWakeAt=now;retailWakePromise=Promise.allSettled([loadRetailOrdersCloud(),loadFinishedStockMovementsCloud()]).finally(()=>{retailWakePromise=null});return retailWakePromise};window.addEventListener('focus',refreshVisibleBakeryCloud);document.addEventListener('visibilitychange',()=>{if(!document.hidden)refreshVisibleBakeryCloud()});window.addEventListener('online',refreshVisibleBakeryCloud);window.panoraRetailFoundation={settings:readRetailSettings,productSettings:readRetailProductSettings,orders:readRetailOrders,render:renderRetailFoundation,loadCloud:cloudLoad,loadOrders:loadRetailOrdersCloud,loadFinishedStock:loadFinishedStockMovementsCloud,location:'bakery'};
}
...
/* Panora 7.02 — freeze the raw-material cost together with the factual bake.
   The bake_completions.items column is JSON, so this adds audit data without SQL. */
function bakeCostNormalizeName(value){return String(value||'').trim().toLocaleLowerCase('ru-RU').replace(/ё/g,'е').replace(/[‐‑‒–—]/g,'-').replace(/\s+/g,' ')}
function bakeCostNormalizeUnit(value){return String(value||'').trim().toLowerCase()}
function bakeCostFactor(unit){return unit==='g'||unit==='ml'?1000:1}
function bakeIngredientPrice(priceMap,name,unit){return Number(priceMap[`${bakeCostNormalizeName(name)}|${bakeCostNormalizeUnit(unit)}`]??priceMap[`${name}|${unit}`]??0)}
function bakeRecipeCostSnapshot(product,recipeSnapshot=bakeRecipeSnapshot(product),capturedAt=new Date().toISOString(),source='actual_completion'){
 let priceMap={};try{priceMap=JSON.parse(localStorage.getItem('panora-ingredient-costs')||'{}')||{}}catch{}
 let unitRawCost=0,complete=true;
 const ingredients=(Array.isArray(recipeSnapshot)?recipeSnapshot:[]).map(item=>{
  const qty=Math.max(0,Number(item?.qty||0));if(!qty)return null;
  if(item?.sourceIngredientName&&Number(item?.sourceYieldPct)>0){
   const sourceUnit=bakeCostNormalizeUnit(item.sourceUnit||item.unit||'g')||'g',yieldPct=Number(item.sourceYieldPct),rawQty=qty/(yieldPct/100),unitPrice=bakeIngredientPrice(priceMap,item.sourceIngredientName,sourceUnit),cost=rawQty/bakeCostFactor(sourceUnit)*unitPrice;
   if(!(unitPrice>0))complete=false;unitRawCost+=cost;
   return{name:String(item.name||''),unit:bakeCostNormalizeUnit(item.unit||'g')||'g',qty,sourceIngredientName:String(item.sourceIngredientName||''),sourceUnit,sourceYieldPct:yieldPct,rawQty,unitPrice,cost};
  }
  const unit=bakeCostNormalizeUnit(item.unit||'g')||'g',unitPrice=bakeIngredientPrice(priceMap,item.name,unit),cost=qty/bakeCostFactor(unit)*unitPrice;
  if(!(unitPrice>0))complete=false;unitRawCost+=cost;
  return{name:String(item.name||''),unit,qty,unitPrice,cost};
 }).filter(Boolean);
 return{version:1,product:String(product||''),capturedAt:String(capturedAt||new Date().toISOString()),source:String(source||'actual_completion'),complete:ingredients.length>0&&complete,unitRawCost,ingredients};
}
...
function upgradeBakeCompletionSnapshots(){
 ...
 list.forEach(completion=>{
  ...
  (completion.items||[]).forEach(item=>{
   if(!Array.isArray(item.recipeSnapshot)||!item.recipeSnapshot.length){item.recipeSnapshot=bakeRecipeSnapshot(String(item.product||''));changed=true}
   if(!item.costSnapshot||Number(item.costSnapshot.version||0)<1){item.costSnapshot=bakeRecipeCostSnapshot(String(item.product||''),item.recipeSnapshot,now,'migration_estimate');changed=true}
  });
 });
 if(!changed)return false;
 list.forEach(completion=>{
  if(completion?.deletedAt)return;
  completion.snapshotVersion=2;
...
function renderPlan(){
 ...
 const today=typeof stockLocalDate==='function'?stockLocalDate():iso(new Date()),history=Boolean(window.panoraBakeCalendarHistoryMode);
 const visible=plans.filter(p=>p.bakeDate>=iso(weekStart)&&p.bakeDate<=iso(end)&&(history?p.bakeDate<today:p.bakeDate>=today)).sort((a,b)=>a.bakeDate.localeCompare(b.bakeDate)),partnerOrdered=visible.reduce((sum,p)=>sum+Math.max(0,Number(p.ordered||0)),0),retailOrdered=visible.reduce((sum,p)=>sum+retailPreorderQuantity(p.bakeDate,p.product),0),ordered=partnerOrdered+retailOrdered,planned=visible.reduce((sum,p)=>{const demand=Math.max(0,Number(p.ordered||0))+retailPreorderQuantity(p.bakeDate,p.product);return sum+Math.max(Math.max(0,Number(p.planned||0)),demand)},0);$('#plannedPieces').textContent=`${planned} ${t('pcs')}`;$('#orderedPieces').textContent=`${ordered} ${t('pcs')}`;$('#freePieces').textContent=`${Math.max(0,planned-ordered)} ${t('pcs')}`;
 $('#planList').innerHTML=visible.length?visible.map(p=>{const partner=Math.max(0,Number(p.ordered||0)),retail=retailPreorderQuantity(p.bakeDate,p.product),real=partner+retail,manual=Math.max(0,Number(p.planned||0)),working=Math.max(manual,real),percent=working?Math.min(100,Math.round(real/working*100)):0,completion=bakeCompletionFor(p.bakeDate),readonly=history||p.bakeDate<today,planLabel=manual>0?`${manual} ${t('pcs')}`:(lang==='ru'?'без количества':lang==='es'?'sin cantidad':'no quantity');return `<article class="plan-card ${completion?'bake-is-completed':''} ${readonly?'plan-history-readonly':''}"><div class="date"><strong>${fmt(p.bakeDate,{weekday:'short',day:'numeric',month:'short'})}</strong><small>${t('delivery')}: ${fmt(p.deliveryDate)}</small></div><div class="product"><strong>${productName(p.product)}</strong><small>${t('cutoffShort')}: ${new Date(p.cutoff).toLocaleString()}</small></div><div><strong>${lang==='ru'?'План':lang==='es'?'Plan':'Plan'}: ${planLabel}</strong><small class="plan-demand-sources">${lang==='ru'?'Партнёры':lang==='es'?'Socios':'Partners'} ${partner} · Retail ${retail} · ${lang==='ru'?'Всего заказано':lang==='es'?'Total pedido':'Total ordered'} ${real}</small><div class="progress"><i style="width:${percent}%"></i></div></div><div><strong>${working} ${t('pcs')}</strong><small>${lang==='ru'?'к выпечке сейчас':lang==='es'?'a hornear ahora':'to bake now'}</small></div><span class="status ${completion?'completed':readonly?'history':p.open?'':'closed'}">${completion?'Завершена':readonly?'История':p.open?t('open'):t('closed')}</span><button class="icon-delete" data-cancel-plan-date="${p.bakeDate}" title="${readonly?'История — только просмотр':'Отменить весь день выпечки'}" aria-label="${readonly?'История — только просмотр':'Отменить весь день выпечки'}" ${readonly||completion?'disabled':''}>×</button></article>`}).join(''):`<article class="plan-card"><div>${history?'Нет прошедших выпечек в этой неделе.':t('empty')}</div></article>`;
 $$('[data-cancel-plan-date]').forEach(button=>button.onclick=async()=>{if(button.disabled)return;const date=button.dataset.cancelPlanDate,reason=prompt(`Укажите причину отмены выпечки ${date}:`);if(reason===null)return;if(!reason.trim()){alert('Причина отмены обязательна.');return}if(confirm(`Отменить весь день выпечки ${date}, включая все виды хлеба и неотгруженные заказы?`))await performCancelBake(date,reason.trim())});
...
function ingredientTotals(){const totals={};plans.filter(p=>p.bakeDate>=iso(new Date())&&(purchaseDateFilter==='all'||p.bakeDate===purchaseDateFilter)).forEach(p=>{const demand=Math.max(0,Number(p.ordered||0))+retailPreorderQuantity(p.bakeDate,p.product),pieces=Math.max(Math.max(0,Number(p.planned||0)),demand);(recipes[p.product]||[]).forEach(r=>{const key=`${r.name}|${r.unit}`;totals[key]??={name:r.name,unit:r.unit,required:0,stock:Number(r.stock||0),margin:Number(r.margin||5)};totals[key].required+=pieces*Number(r.qty)})});return Object.values(totals)}
...
function stockRetailCompletedMovements(){
 return readRetailOrders().filter(order=>order&&retailOrderStatus(order)==='completed').flatMap(order=>{
  // Panora 7.11: keep retail stock issue on its local economic day around UTC midnight.
...
function stockMovementNote(m){
 if(m.type==='shipped'&&m.noteId){const label=String(m.note||'Накладная').replace(/\s*·?\s*\[panora:b2b-shipment:[^\]]+\]/g,'').trim()||'Накладная';return `<button type="button" class="stock-doc-link" data-stock-note="${adminEscape(m.noteId)}" data-stock-order="${adminEscape(m.orderId||'')}">${adminEscape(label)}</button>`};
...
(()=>{
 ...
 const syncLanguage=()=>{const current=currentLanguage();menu.querySelectorAll('[data-admin-language]').forEach(button=>{const active=button.dataset.adminLanguage===current;button.classList.toggle('active',active);button.setAttribute('aria-pressed',active?'true':'false')});if(settingsLanguage&&settingsLanguage.value!==current)settingsLanguage.value=current};
 ...
 settingsLanguage?.addEventListener('change',()=>{if(!language)return;language.value=settingsLanguage.value;language.dispatchEvent(new Event('change',{bubbles:true}));syncLanguage();syncPush()});
...
function setDefaultPlanDates(form,date){form.deliveryDate.value=date;const cutoff=new Date(`${date}T09:00:00`);cutoff.setHours(cutoff.getHours()-48);form.cutoff.value=localDateTimeInputValue(cutoff)}
...
$('#planForm').onsubmit=e=>{e.preventDefault();const form=$('#planForm');if(typeof window.panoraSaveBakePlanFromForm==='function'){window.panoraSaveBakePlanFromForm(form);return}const error=$('#planError');if(error)error.textContent='Обновите приложение и повторите сохранение дня выпечки.'};
...
async function performCancelBake(date,reason){if(bakeCompletionFor(date)){alert('Выпечка уже завершена. Отмена завершённого дня недоступна; при необходимости измените факт выпечки.');return false}const stats=cancelBakeStats(date),cancelledAt=new Date().toISOString();if(!stats.dayPlans.length){alert('Эта дата уже отменена или отсутствует в плане.');return false}const cloudReady=Boolean(navigator.onLine&&window.panoraCloud?.ready&&typeof window.panoraCloud.cancelBakeDayAtomic==='function');if(cloudReady){try{await window.panoraCloud.cancelBakeDayAtomic({date,reason})}catch(error){console.error('Panora cancel bake day',error);alert(`Не удалось отменить день выпечки в облаке. ${error?.message||error}`);return false}}stats.affected.forEach(o=>{o.status='cancelled';o.cancellationReason=reason;o.cancelledAt=cancelledAt});plans=plans.filter(p=>p.bakeDate!==date);const log=read('panora-cancelled-bake-dates',[]);log.push({date,reason,cancelledAt,orders:stats.affected.map(o=>o.id),pieces:stats.pieces});localStorage.setItem('panora-cancelled-bake-dates',JSON.stringify(log));saveAdminOrdersCache(stats.orders);store('panora-production-plans',plans);renderAll();if(typeof renderCommerce==='function')renderCommerce();alert(cloudReady?`Выпечка ${date} отменена. Отменено заказов: ${stats.affected.length}.`:`Выпечка ${date} отменена на устройстве. При восстановлении интернета Panora отменит связанные заказы и уберёт день из облака.`);return true}
...
async function retailAdminPushStatus(){
 const settings=readRetailSettings();
 if(!('Notification'in window)||!('serviceWorker'in navigator)||!('PushManager'in window))return{active:false,reason:'unsupported'};
 if(!settings.pushAdminEnabled)return{active:false,reason:'disabled_by_settings'};
 if(!settings.pushVapidPublicKey)return{active:false,reason:'missing_vapid'};
 if(Notification.permission!=='granted')return{active:false,reason:Notification.permission};
 if(!window.panoraSupabaseSession?.access_token)return{active:false,reason:'no_session'};
 try{
  const reg=await navigator.serviceWorker.ready,sub=await reg.pushManager.getSubscription();
  if(!sub)return{active:false,browser:false,server:false,reason:'no_subscription'};
  const rows=await retailAdminApi('rpc/panora_admin_push_status',{method:'POST',body:JSON.stringify({p_endpoint:sub.endpoint})});
  const row=Array.isArray(rows)?rows[0]:rows,active=Boolean(row?.active??row);
  return{active,browser:true,server:active,reason:active?'ready':'not_registered',endpoint:sub.endpoint};
 }catch(error){return{active:false,browser:true,server:false,reason:'status_error',error:String(error?.message||error)}}
}
function retailRenderAdminPushState(info){
 const state=$('#retailAdminPushState'),settingsButton=$('#retailEnableAdminPush'),test=$('#retailTestPush');
 const active=Boolean(info?.active);
 if(state)state.textContent=active?'Push подключён на этом устройстве ✓':
  info?.reason==='denied'?'Push запрещён в настройках браузера':
  info?.reason==='missing_vapid'?'Добавьте VAPID public key в настройках':
  info?.reason==='disabled_by_settings'?'Push пекарне выключен в настройках':
  info?.reason==='unsupported'?'Этот браузер не поддерживает Web Push':
  'Push не подключён на этом устройстве';
 if(settingsButton)settingsButton.textContent=active?'Отключить Push на устройстве':'Включить Push на устройстве';
 if(test)test.disabled=!active;
 try{
  if(active)localStorage.setItem('panora-admin-webpush-registered','1');
  else localStorage.removeItem('panora-admin-webpush-registered');
 }catch{}
 window.dispatchEvent(new CustomEvent('panora:admin-webpush-state',{detail:{active,reason:info?.reason||''}}));
 return info;
}
async function retailEnsureAdminPush({prompt=false}={}){
 const settings=readRetailSettings();
 if(!('Notification'in window)||!('serviceWorker'in navigator)||!('PushManager'in window))return retailRenderAdminPushState({active:false,reason:'unsupported'}),false;
 if(!settings.pushAdminEnabled)return retailRenderAdminPushState({active:false,reason:'disabled_by_settings'}),false;
 if(!settings.pushVapidPublicKey)return retailRenderAdminPushState({active:false,reason:'missing_vapid'}),false;
 try{
  let permission=Notification.permission;
  if(permission!=='granted'&&prompt)permission=await Notification.requestPermission();
  if(permission!=='granted'){retailRenderAdminPushState({active:false,reason:permission});return false}
  if(!window.panoraSupabaseSession?.access_token)return false;
  ...
  retailRenderAdminPushState({active:false,reason:'error',error:String(error?.message||error)});
  if(prompt)throw error;return false;
 }
}
async function retailDisableAdminPush(){
 if(!('serviceWorker'in navigator)||!window.panoraSupabaseSession?.access_token)return false;
 try{
  const reg=await navigator.serviceWorker.ready,sub=await reg.pushManager.getSubscription();
  if(sub)await retailAdminApi('rpc/panora_admin_disable_push',{method:'POST',body:JSON.stringify({p_endpoint:sub.endpoint})});
  retailRenderAdminPushState({active:false,reason:'disabled'});
  return true;
 }catch(error){retailRenderAdminPushState({active:false,reason:'error',error:String(error?.message||error)});throw error}
}
async function retailToggleAdminPush(){
 const status=await retailAdminPushStatus();
 return status.active?retailDisableAdminPush():retailEnsureAdminPush({prompt:true});
}
async function retailEnableAdminPush(){return retailEnsureAdminPush({prompt:true})}
window.panoraAdminWebPush={enable:retailEnableAdminPush,disable:retailDisableAdminPush,toggle:retailToggleAdminPush,status:retailAdminPushStatus,ensure:()=>retailEnsureAdminPush({prompt:false})};

async function retailSendTestPush(){
 const state=$('#retailAdminPushState'),button=$('#retailTestPush');
 if(button)button.disabled=true;
 try{
  const ready=await retailAdminPushStatus();
  if(!ready.active)throw new Error('Сначала включите Push на этом устройстве');
