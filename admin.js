...
/* Panora 6.34 — in-app retail messages + fallback messenger contacts. */
...
async function loadRetailSettingsCloud(){
 let rows=null,level='634';
 try{rows=await retailAdminApi('retail_settings?id=eq.1&select=id,enabled,stock_sales,preorders,pickup,delivery,online_payment,pay_on_pickup,reservation_minutes,preorder_cutoff_hours,preorder_horizon_days,pickup_lead_minutes,max_orders_per_slot,pickup_slots,contact_phone,contact_email,contact_whatsapp,contact_telegram,fallback_whatsapp,fallback_telegram,fallback_sms,fallback_email,pickup_note,payment_provider,delivery_min_total,delivery_fee,delivery_free_from,max_deliveries_per_slot,delivery_slots,notify_order_received,notify_ready,notify_delivery,notify_cancelled,updated_at&limit=1')}
 catch(error){
  level='633';
 ...
 if(rows?.[0])saveRetailSettingsLocal(retailSettingsFromCloud(rows[0]));renderRetailFoundation();const saved=$('#retailSettingsSaved');if(saved&&rows?.[0])saved.textContent=level==='634'?'Настройки и каналы связи загружены из облака':level==='633'?'Настройки загружены · выполните SQL 6.34 для сообщений':level==='628'?'Настройки загружены · выполните SQL 6.33 и 6.34':'Обновите SQL розницы';return !!rows?.[0];
}
...
function syncRetailSettingsForm(settings){const form=$('#retailSettingsForm');if(!form)return;['enabled','stockSales','preorders','pickup','delivery','onlinePayment','payOnPickup','notifyOrderReceived','notifyReady','notifyDelivery','notifyCancelled','fallbackWhatsApp','fallbackTelegram','fallbackSms','fallbackEmail'].forEach(name=>{if(form.elements[name])form.elements[name].checked=!!settings[name]});['reservationMinutes','preorderCutoffHours','preorderHorizonDays','pickupLeadMinutes','maxOrdersPerSlot','deliveryMinTotal','deliveryFee','deliveryFreeFrom','maxDeliveriesPerSlot','contactPhone','contactEmail','contactWhatsApp','contactTelegram','pickupNote','paymentProvider'].forEach(name=>{if(form.elements[name])form.elements[name].value=String(settings[name]??'')});if(form.elements.pickupSlots)form.elements.pickupSlots.value=(settings.pickupSlots||[]).join('\n');if(form.elements.deliverySlots)form.elements.deliverySlots.value=(settings.deliverySlots||[]).join('\n');const label=$('#retailEnabledLabel');if(label)label.textContent=settings.enabled?'Включена':'Выключена'}
