...
/* Panora 6.25 — retail preorders integrated with production, raw-material demand and finished-bread reserve. One and only location: bakery. */
...
async function loadRetailSettingsCloud(){
 ...
 catch(error){extended=false;try{rows=await retailAdminApi('retail_settings?id=eq.1&select=id,enabled,stock_sales,preorders,pickup,delivery,online_payment,reservation_minutes,preorder_cutoff_hours,pickup_slots,updated_at&limit=1')}catch(second){const saved=$('#retailSettingsSaved');if(saved)saved.textContent='Локальные настройки · выполните SQL 6.25';return false}}
 if(rows?.[0])saveRetailSettingsLocal(retailSettingsFromCloud(rows[0]));renderRetailFoundation();const saved=$('#retailSettingsSaved');if(saved&&rows?.[0])saved.textContent=extended?'Настройки загружены из облака':'Выполните SQL 6.25 для связи розницы с производством';return !!rows?.[0];
}
...
async function loadRetailProductSettingsCloud(){
 ...
 catch(error){const status=$('#retailCatalogSaved');if(status)status.textContent='Локальный ассортимент · выполните SQL 6.25';return false}
}
...
async function loadRetailOrdersCloud(){
 ...
 catch(error){if(status)status.textContent='Выполните SQL 6.25';return false}
}
