...
/* Panora 6.26 — retail order status page + safe public token access; production integration from 6.25 preserved. One and only location: bakery. */
...
async function loadRetailSettingsCloud(){
 ...
 catch(error){extended=false;try{rows=await retailAdminApi('retail_settings?id=eq.1&select=id,enabled,stock_sales,preorders,pickup,delivery,online_payment,reservation_minutes,preorder_cutoff_hours,pickup_slots,updated_at&limit=1')}catch(second){const saved=$('#retailSettingsSaved');if(saved)saved.textContent='Локальные настройки · выполните SQL 6.26';return false}}
 if(rows?.[0])saveRetailSettingsLocal(retailSettingsFromCloud(rows[0]));renderRetailFoundation();const saved=$('#retailSettingsSaved');if(saved&&rows?.[0])saved.textContent=extended?'Настройки загружены из облака':'Выполните SQL 6.26 для связи розницы с производством';return !!rows?.[0];
}
...
async function loadRetailProductSettingsCloud(){
 ...
 catch(error){const status=$('#retailCatalogSaved');if(status)status.textContent='Локальный ассортимент · выполните SQL 6.26';return false}
}
...
async function loadRetailOrdersCloud(){
 ...
 catch(error){if(status)status.textContent='Выполните SQL 6.26';return false}
}
