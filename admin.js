...
/* Panora 6.27 — retail payment architecture. Payment state is separate from fulfilment; one location: bakery. */
...
function retailCloudRow(settings,{legacy=false}={}){
 ...
 if(!legacy)Object.assign(row,{pay_on_pickup:!!settings.payOnPickup,preorder_horizon_days:settings.preorderHorizonDays,pickup_lead_minutes:settings.pickupLeadMinutes,max_orders_per_slot:settings.maxOrdersPerSlot,
  contact_phone:settings.contactPhone||null,contact_email:settings.contactEmail||null,pickup_note:settings.pickupNote||null,payment_provider:settings.paymentProvider||'none'});
 return row;
}
function retailSettingsFromCloud(row){if(!row)return null;return normalizeRetailSettings({enabled:row.enabled,stockSales:row.stock_sales,preorders:row.preorders,pickup:row.pickup,delivery:row.delivery,onlinePayment:row.online_payment,payOnPickup:row.pay_on_pickup,
 ...
 pickupSlots:Array.isArray(row.pickup_slots)?row.pickup_slots:RETAIL_DEFAULT_SETTINGS.pickupSlots,contactPhone:row.contact_phone,contactEmail:row.contact_email,pickupNote:row.pickup_note,paymentProvider:row.payment_provider})}
...
async function loadRetailSettingsCloud(){
 let rows=null,extended=true;try{rows=await retailAdminApi('retail_settings?id=eq.1&select=id,enabled,stock_sales,preorders,pickup,delivery,online_payment,pay_on_pickup,reservation_minutes,preorder_cutoff_hours,preorder_horizon_days,pickup_lead_minutes,max_orders_per_slot,pickup_slots,contact_phone,contact_email,pickup_note,payment_provider,updated_at&limit=1')}
 catch(error){extended=false;try{rows=await retailAdminApi('retail_settings?id=eq.1&select=id,enabled,stock_sales,preorders,pickup,delivery,online_payment,reservation_minutes,preorder_cutoff_hours,pickup_slots,updated_at&limit=1')}catch(second){const saved=$('#retailSettingsSaved');if(saved)saved.textContent='Локальные настройки · выполните SQL 6.27';return false}}
 if(rows?.[0])saveRetailSettingsLocal(retailSettingsFromCloud(rows[0]));renderRetailFoundation();const saved=$('#retailSettingsSaved');if(saved&&rows?.[0])saved.textContent=extended?'Настройки загружены из облака':'Выполните SQL 6.27 для связи розницы с производством';return !!rows?.[0];
}
...
async function loadRetailProductSettingsCloud(){
 ...
 catch(error){const status=$('#retailCatalogSaved');if(status)status.textContent='Локальный ассортимент · выполните SQL 6.27';return false}
}
