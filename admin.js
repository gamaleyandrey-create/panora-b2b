...
async function loadFinishedStockMovementsCloud(){
 try{
  ...
  movements=[...byId.values()].filter(Boolean);localStorage.setItem('panora-stock-movements',JSON.stringify(movements));renderStock();window.dispatchEvent(new CustomEvent('panora:finished-stock-cloud-updated'));return true;
 }catch{return false}
}
