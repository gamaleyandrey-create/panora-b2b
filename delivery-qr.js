...
(function(){
  ...
  const tr=value=>window.panoraAdminTranslateText?.(value)||value;
  function token(note){
  ...
  function url(note){const value=new URL('confirm.html',document.baseURI);value.searchParams.set('t',token(note));const client=typeof restaurant==='function'?restaurant(note.restaurantId):null;const qrLang=['ru','en','es'].includes(String(client?.language||'').toLowerCase())?String(client.language).toLowerCase():(document.querySelector('#adminLanguage')?.value||'ru');value.searchParams.set('lang',qrLang);return value.href}
  ...
  window.showDeliveryQr=async orderId=>{
    const note=deliveryNotes.find(n=>n.orderId===orderId),order=orders.find(o=>o.id===orderId);if(!note)return alert(tr('Поставка не найдена.'));
    let code;try{code=await qr(note)}catch(error){console.error(error);alert(tr('Не удалось сформировать QR-код. Обновите страницу и попробуйте ещё раз.'));return}
    ...
    document.body.insertAdjacentHTML('beforeend',`<dialog id="deliveryQrDialog" class="modal delivery-qr-modal"><form method="dialog" class="dialog-card"><button class="modal-close" type="button" aria-label="${esc(tr('Закрыть'))}">×</button><p class="eyebrow">${esc(tr('PANORA · ПОСТАВКА'))}</p><h2>${esc(tr('Подтверждение получения'))}</h2><p><strong>${esc(client?.name)}</strong> · PN-${String(order?.number||'—').padStart(4,'0')}</p><img class="delivery-qr-image" src="${code.image}" alt="QR-код"><p class="delivery-qr-help"><strong>${esc(tr('Дата поставки'))}: ${esc(deliveryLabel)}</strong></p><div class="delivery-qr-items">${note.items.map(i=>`<span>${esc(name(i.product))}</span><strong>${i.quantity} ${esc(tr('шт.'))}</strong>`).join('')}</div><div class="delivery-qr-actions"><button class="primary qr-close" type="button">${esc(tr('Готово'))}</button><button class="secondary qr-invoice" type="button">${esc(tr('Открыть накладную'))}</button></div></form></dialog>`);
  ...
  window.printNote=async function(orderId){
    ...
    try{
      ...
      if(sheet&&!sheet.querySelector('.invoice-qr')){
        ...
        footer?.insertAdjacentHTML('beforebegin',`<section class="invoice-qr" aria-label="${esc(tr('QR-код подтверждения поставки'))}"><img src="${code.image}" alt="${esc(tr('QR-код подтверждения поставки'))}"></section>`);
      }
    }catch(error){console.error('Invoice QR:',error)}
    return result;
  };
  const oldOpen=window.openShipment;
  window.openShipment=function(id){oldOpen(id);const s=document.querySelector('#shipmentSummary');if(s&&!s.querySelector('.qr-hint'))s.insertAdjacentHTML('beforeend',`<p class="qr-hint">${esc(tr('После отгрузки откроется защищённый QR-код. Накладная останется отдельной кнопкой.'))}</p>`)};
})();
