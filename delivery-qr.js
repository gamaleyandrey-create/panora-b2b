  ...
  window.printNote=async function(orderId){
    ...
    try{
      ...
      if(sheet&&!sheet.querySelector('.invoice-qr')){
        ...
        footer?.insertAdjacentHTML('beforebegin',`<section class="invoice-qr"><img src="${code.image}" alt="QR-код подтверждения поставки"><div><strong>Подтверждение получения</strong><p>Отсканируйте QR-код в кабинете ресторана. Код открывает только накладную этого ресторана.</p><small>${esc(code.link)}</small></div></section>`);
      }
